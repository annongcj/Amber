...
! DEFINES
  ...
  public NC_create, NC_setupAmberNetcdf, NetcdfFileExists, NC_error, &
         ...
         GetDimInfo,  NC_setupTraj, NC_readTrajBox, NC_setupBox
...
!> @detail Technically a title is Not required, see issue #242;
!!         nevertheless, emit a note when one is absent.
!> @param ncid Netcdf ID of opened file.
!> @return true if error, false if not.
logical function NC_GetTitle(ncid, title)
  ...
  if (err .eq. NF90_ENOTATT) then
    ! This is Not an error state; emit, fall through, and exit normally.
    if (verbose_netcdf) then
      write(mdout, '(a,a)') 'NetCDF note:  A title was not found.'
    end if
  else if (NC_error( err, 'Getting title')) then
    return
  endif
  ! Exit normally.
  NC_GetTitle=.false.
...
integer function NC_setupRemdTypes(ncid)
  ...
  NC_setupRemdTypes=-1
  if ( nf90_inq_varid(ncid,NCREMD_DIMTYPE,RemdTypesVID) .eq. NF90_NOERR ) then
#   ifdef _NETCDF_DEBUG
...
# ifdef _NETCDF_DEBUG
  write(mdout,'(a,a,a,i8,5(a,l1))') "| DEBUG: NC_create: ", trim(filename), &
        "  natom=", natomIn, "  V=", hasVelocity, "  box=", hasBox, &
        "  remd_values=", hasRemdValues, "  time=", hasTime, "  F=", hasFrc
# endif
...
logical function NC_setupReservoir(ncid, reservoirsize, restemp0, numatoms,&
  ...
  ! Get reservoir temp
  if (NC_error(nf90_inq_varid(ncid, NCTEMPERATURE, TempVID),&
              'Getting netcdf reservoir temperature VID')) return
...
logical function NC_defineRemdIndices(ncid, remd_dimension, indicesVID, &
                                      repidxVID, crdidxVID, &
                                      remd_types, isRestart, isREMD, &
                                      isMREMD, frameDID, remd_groupsVID, &
  ...
  logical, intent(in)               :: isRestart
  logical, intent(in)               :: isREMD,isMREMD
  integer, optional, intent(in)     :: frameDID
  integer, optional, intent(out)    :: remd_groupsVID
  integer, intent(out)              :: remd_valuesVID, remd_typesVID
  ! Local vars
  integer, dimension(NF90_MAX_DIMS) :: dimensionID
  integer :: remd_dim_id, old_mode
  ...
  if (isREMD) then
    ...
    remd_typesVID = NC_define_var(ncid, NCREMD_DIMTYPE, NF90_INT, &
                                        1, dimensionID)
    if (remd_typesVID.eq.-1) return ! 'define replica type for each dimension'
    if (isRestart) then
  ...
  if (isREMD) then
    ...
    if (NC_error(nf90_put_var(ncid, remd_typesVID, remd_types(:), &
...
!> MODULE AMBERNETCDF FUNCTION NC_SETUPTRAJ 
logical function NC_setupTraj(ncid, title, ncatom, coordVID, &
                                 velocityVID, TempVID, Time)
  use netcdf
  implicit none
  integer, intent(in)       :: ncid
  ...
  NC_setupTraj=.true.
  ! Check this is actually a trajectory
  if (NC_checkConventions(ncid, .false.)) return
  ! Read title
  if (NC_GetTitle(ncid, title)) return
  ! number of atoms, coordVID and velocityVID
  ...
  ! Setup time. Allowed to fail since not all restarts have this.
  if (NC_setupTime(ncid, timeVID)) then
    write(mdout,'(a)') 'Warning: NetCDF traj has no time value.'
    Time = 0.d0
    timeVID = -1
  else
    if (NC_error(nf90_get_var(ncid, timeVID, Time),&
...
!--------------------------------------------------------------------
!> MODULE AMBERNETCDF FUNCTION NC_READTRAJBOX
!> @brief Read box information from previously opened netcdf restart.
logical function NC_readTrajBox(ncid,a,b,c,alpha,beta,gamma,frame)
  use netcdf
  implicit none
  integer, intent(in)           :: ncid
  double precision, intent(out) :: a,b,c,alpha,beta,gamma
  ...
  ! Cell angles
  if (NC_error(nf90_get_var(ncid, cellAngleVID, box(1:3), &
                            start = (/ 1,1,frame /), count = (/ 3,1 /)),&
               'Getting box angles')) return
  alpha = box(1)
  beta  = box(2)
  gamma = box(3)
  ! All is well
  NC_readTrajBox=.false.
end function NC_readTrajBox
