...
group.add_argument('-nosort', dest='nosort', action='store_const',
                   ...
                   const=True, default=False)
group.add_argument('-precsuffix', dest='precsuffix', metavar='INTEGER',
                   help='''Number of digits to be used in the suffix added to the generated files. Default: 3''',
                   type=int, default=3)
...
def main(opt):
    ...
    # Number of reference mdins
    if (opt.mdins):
        nrefmdins = len(opt.mdins)
    else:
        nrefmdins = 0
    ...
    if (opt.groupfiles):
        ...
        nrefgrpfls = 0
    # Number of reference disangs
    if (opt.disangs):
        nrefdisangs = len(opt.disangs)
    else:
        nrefdisangs = 0
    ...
    for i in range(ndims):
        ...
        while True:
            ...
            if (line):
                ...
                break
            cnt += 1
        if(opt.groupfiles and len(vector)%2 != 0): ErrorExit('The number of %s values (%d) in the file %s is not even! The number of replicas on each REMD dimension needs to be even'%(remd_txttypes[i],len(vector),opt.inputs[i]))
        if (opt.nosort):
            lists.append(vector)
        else:
    ...
    for i in range(nrefgrpfls):
        ...
        group_file.close()
    if (nrefgrpfls > 0): del group_file,numblocks,FoundFlag

    # Checking on the DISANG file(s) if the required pointer are there
    if (opt.disangs):
        disanglines = []
        for i in range(nrefdisangs):
    ...
    if (opt.disangs):
        disangfiles = [["" for rep in range(nreps)] for i in range(nrefdisangs)]
        for i in range(nrefdisangs):
            ...
            for rep in range(nreps):
                filename = txt+'.rep.{val:0{valsize}d}'.format(val=rep+1,valsize=opt.precsuffix)
    ...
    mdinfiles = [["" for rep in range(nreps)] for i in range(nrefmdins)]
    for i in range(nrefmdins):
    ...
    for rep in range(nreps):
        ...
        for i in range(nrefgrpfls):
            ...
            for line in grpfllines[i]:
                line = line.replace('REPNUM','{val:0{valsize}d}'.format(val=rep+1,valsize=opt.precsuffix))
                if (DoingHamiltonian):
                    for j in range(len(hdims)):
        ...
        for i in range(nrefmdins):
            ...
            for line in mdinlines[i]:
                for dim in range(ndims+2):
                    if (dim == ndims+1):
                        flag = "REPNUM"
                        val = str('{val:0{valsize}d}'.format(val=rep+1,valsize=opt.precsuffix))
                    elif (dim == ndims):
                        flag = "RANDOMNUM"
                        val = str(randomnum)
                    else:
