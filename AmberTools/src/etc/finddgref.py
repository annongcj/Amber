...
def RunAMBER(opt,log,data,equi):
    ...
    if (equi != 1):
        ...
        if (data.remd):
            ...
            lines = str(pipe.stdout.readlines())
...
def EstimateDG(opt,log,data):
    ...
    if (data.step == 1):
        PrintLog(opt,log," We start with accurate estimatives of DELTAGREF values starting from the value given at the argument -dgrefest (%.6f kcal/mol)." % data.dg)
    else:
        PrintLog(opt,log,"\n From this step on, we will make more accurate estimatives of DELTAGREF values.")
    if (data.modeopt == 1):
        if (data.remd):
