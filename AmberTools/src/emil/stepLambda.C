...
//try cranking up the mix order?
#define MIX_ORDER_MOL  4
//#define MIX_ORDER_MOL 6
...
double hsc::mixFunc_soft(double lambda){
  ...
  //experimental mixing function: intended to ease the transition at high lambda, 
  //for use in conjunction with softcoring.
  double scale;
...
double hsc::genForce_mol(double dvdl, double emodel, double lambda ){
  ...
  if ( amberSoftcoring || !emilSoftForce) return( dvdl );
