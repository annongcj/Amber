...
*logFile<<"# " << invNheavy_solid << " heavy or solute atoms." << endl;
  ...
  if( nSwap == 0 && swapTriesPerChain > 0.  && numLiquidChains > 0 ){
     nSwap += 1;
  }
*logFile<<"# Making " << nSwap << " id-swap moves per MD step." << endl;

  nReloc   = int(  numLiquidChains * relocTriesPerChain );
  if( nReloc == 0 && relocTriesPerChain > 0.  && numLiquidChains > 0 ){
     nReloc += 1;
  }
...
void hsc::initWells() {
  ...
  //  w = new wingWell( req, rcutLiquid, wingForce );
   
    w = new wingWell( req, rcutLiquid, epsilonTrap );
    outfile << "#wing well set up with: " << req << " " << rcutLiquid << " " << epsilonTrap << "\n";
    w->testMe( &outfile );
    delete w;
  ...
  for( i = 0; i < N; i++) {
      ...
      switch( part[i].wellType ){
        ...
        case HSC_POTENTIAL_CONEWELL_LIQUID:
          ...
          cerr << "coneWell potential is deprecated, EMIL quitting" << endl;
          exit(8);
        ...
        case HSC_POTENTIAL_HARMONIC_LIQUID:
          ...
          wells[i]->req2     =  wells[i]->req * wells[i]->req;
            ...
            //non-root particles are attracted to their root, not to an absolute well.
            part[i].wellType = HSC_POTENTIAL_RELATIVE_POS;
            wells[i]         = new harmonicBondWell; 
        ...
        case HSC_POTENTIAL_WINGWELL_LIQUID:
          if( part[i].isRoot ){
            wells[i] = new wingWell( req, rcutLiquid, epsilonTrap );
            //wells[i] = new wingWell( req, rcutLiquid, wingForce  ); 
          }else{
        ...
        case HSC_POTENTIAL_SOFTSTEP_LIQUID:
          if( part[i].isRoot ){
	   
	    cerr << "setting up softStepWell, req: " << req <<
	            " rcut: " << rcutLiquid << endl;
            wells[i] = new softStepWell( req, rcutLiquid, softStep_A, box->V ); 
	    
	    //finding by quadrature, so best to only do once.
	    if( softStep_A == 0. ){
	      softStep_A = wells[i]->reportFreeEnergy( box->V );
	    }
	    
	    cerr << "has A: " << softStep_A << endl;
	    wells[i]->report(&cerr);
	    
	    cerr << "This material not fully developed, quitting." << endl;
	    exit(8);
  ...
  //try removing well clashes
  double delta_well = 999.; 
  double well_clash_d, well_clash_d2;

  well_clash_d  = 1.25;
  well_clash_d2 = well_clash_d * well_clash_d;
  
  while( delta_well > 1e-8 && 0 ){
     double rforce[N*3], dr, mean_nn;
     double nn[N];
     delta_well = 0.;
     for (int i=0; i<N; i++){
           rforce[i*3+0] = 0.;
     ...
     for (int i=0; i<N-1; i++){
        if( part[i].wellType == HSC_POTENTIAL_EINSTEIN_CRYSTAL || part[i].isRoot ){
           for (int j=i+1; j<N; j++){
               if( part[j].wellType == HSC_POTENTIAL_EINSTEIN_CRYSTAL || part[j].isRoot ){
                 dr  = pow(wells[i]->R[0]-wells[j]->R[0],2);
                 dr += pow(wells[i]->R[1]-wells[j]->R[1],2);
                 dr += pow(wells[i]->R[2]-wells[j]->R[2],2);
                 if( dr < nn[i]*nn[i] ) nn[i] = sqrt(dr);
                 if( dr < nn[j]*nn[j] ) nn[j] = sqrt(dr);
                 if( dr >= well_clash_d2 ) continue;
                 dr          = sqrt(dr);
                 delta_well += (well_clash_d - dr);
                 rforce[i*3+0] =  0.025 * dr * (wells[i]->R[0]-wells[j]->R[0]);
                 rforce[i*3+1] =  0.025 * dr * (wells[i]->R[1]-wells[j]->R[1]);
                 rforce[i*3+2] =  0.025 * dr * (wells[i]->R[2]-wells[j]->R[2]);
                 rforce[j*3+0] = -0.025 * dr * (wells[i]->R[0]-wells[j]->R[0]);
                 rforce[j*3+1] = -0.025 * dr * (wells[i]->R[1]-wells[j]->R[1]);
                 rforce[j*3+2] = -0.025 * dr * (wells[i]->R[2]-wells[j]->R[2]);
               }
           }
        }
     }
     ...
     int count_nn = 0;
     for (int i=0; i<N; i++){
        wells[i]->R[0] += rforce[i*3+0];  
        wells[i]->R[1] += rforce[i*3+1];  
        wells[i]->R[2] += rforce[i*3+2];  
        if( part[i].wellType == HSC_POTENTIAL_EINSTEIN_CRYSTAL || part[i].isRoot ){
           mean_nn        += nn[i];
           count_nn       += 1;
        }
     ...
     if( delta_well > 0. ){
          MERR    << "relaxing-out well position clash, delta is " << delta_well 
                  << " mean nn distance: " << (mean_nn/count_nn) << endl;
         *logFile << "relaxing-out well position clash, delta is " << delta_well 
                  << " mean nn distance: " << (mean_nn/count_nn) << endl;
     }
  }
   
  //try removing outlier relative-position bond vectors
  {
   double mean_wellVec = 0.;
   int    count_wellVec = 0;
   for (int i=0; i<N-1; i++){
      if( part[i].wellType != HSC_POTENTIAL_EINSTEIN_CRYSTAL &&  !part[i].isRoot ){
          double dr2;
          dr2  = wells[i]->R[0]*wells[i]->R[0];
          dr2 += wells[i]->R[1]*wells[i]->R[1];
          dr2 += wells[i]->R[2]*wells[i]->R[2];
          count_wellVec   += 1;
          mean_wellVec    += sqrt(dr2);
      }
   }
   mean_wellVec /= count_wellVec;
   for (int i=0; i<N-1; i++){
      if( part[i].wellType != HSC_POTENTIAL_EINSTEIN_CRYSTAL &&  !part[i].isRoot ){
          double dr2;
          dr2  = wells[i]->R[0]*wells[i]->R[0];
	  ...
	  //see if shortening the bonds a bit helps with anything
          wells[i]->R[0] *= mean_wellVec/dr2;
          wells[i]->R[1] *= mean_wellVec/dr2;
          wells[i]->R[2] *= mean_wellVec/dr2;
      }
   }
  }
