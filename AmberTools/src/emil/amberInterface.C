...
void emil_forces(double *crd, double *frc, double *box, double *dvdl, 
  ...
  if( pmemd_called )
   ...
   //soft force and mol force calculations will do nothing (return directly)
   //if the softcoring is done AMBER-side instead (probably best)
   //  : these functions check the flag "emilSoftForce" to see if they have to
   //  do anything
...
//function to attempt MC moves: doesn't alter coordinates, but alters potential instead.
 ...
 *emil_mc_done = 1;
  ...
  if( (Hsc.sim_nStep+1) % Hsc.printEvery == 0 ){
  ...
  if( Hsc.nSwap  > 0 && (Hsc.sim_nStep+1) % (Hsc.printEvery*10) == 0 )
