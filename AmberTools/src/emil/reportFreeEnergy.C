...
//
// Caution: in some places the code has moved on, but there
// is not yet a new publication.  In these cases, the reasoning
// of integrating over the potential to find the partition
...
double hsc::reportSystemEnthalpy(){
    ...
    *logFile << "#\n";
    *logFile << "# (mixed)  ABS_ENTH(kT) " << showpoint << abstractEnergy * mixFunc_abs( restLambda );
...
<< endl;
    ...
    *logFile << "#\n";
    ...
    if ( ! amberSoftcoring ){
        ...
        return( emolec * beta + abstractEnergy * mixFunc_abs( restLambda ));
    }
