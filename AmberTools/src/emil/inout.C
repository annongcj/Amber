...
//this gets propagated to the relative-position restraints
//which are applied to the all-important case of TIP3P
//hydrogen atoms (which have no VDW radius).
//
// Setting a high default because it is *very* problematic
// when the water H atoms go walkabout due to their total 
// lack of self-protection from Coulomb overlap.
//
  //epsilonSolid    =   5.0;
  epsilonSolid    =  2.0;
  rcutSolid       =  0.1;
  ...
  //Good defaults for wingwell/Einstein mole:
  req             =   0.5;
  epsilonTrap     =   4.0;
  rcutLiquid      =   5.0;
  
  //Defaults for HSC_POTENTIAL_SOFTSTEP_LIQUID: ?
  //req             =   0.1;
  //rcutLiquid      =   0.5;
  ...
  while (infile.peek() != EOF) {
    ...
    if (strcmp(quantity,"number")==0) {
      ...
      //materialType[numMaterials] = HSC_POTENTIAL_SOFTSTEP_LIQUID;
      materialAtomMask[numMaterials].assign(value);
      numMaterials++;
    }
