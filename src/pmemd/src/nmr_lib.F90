...
subroutine angnrg(crd, frc, dfr, thet, i1, i2, i3, e, r1, r2, r3, r4, k2, k3, &
...
! angle (DF):
  ...
  if (ti_mode .eq. 1) then
    if (res_ti_region .ne. 0) then
      df = df * ti_weights(res_ti_region)
    end if
  end if
...
! 1.0 otherwise.
  ...
  frc(i1 + 1) = frc(i1 + 1) - dfr(1)
  frc(i1 + 2) = frc(i1 + 2) - dfr(2)
  frc(i1 + 3) = frc(i1 + 3) - dfr(3)
  frc(i2 + 1) = frc(i2 + 1) - dfr(4)
  frc(i2 + 2) = frc(i2 + 2) - dfr(5)
  frc(i2 + 3) = frc(i2 + 3) - dfr(6)
  frc(i3 + 1) = frc(i3 + 1) - dfr(7)
  frc(i3 + 2) = frc(i3 + 2) - dfr(8)
  frc(i3 + 3) = frc(i3 + 3) - dfr(9)
...
!   ifxyz(5) = coefficient for restricting restraints in x-, y- and/or
!   z-dimension(s). Value should be between 0 and 1. Default = 1. 4th and 5th
!   terms used for single distance restraints.
!
! (the following only used when time-averaging):
!
...
subroutine disnrg(crd, frc, dfr, rij, i1, i2, e, r1, r2, r3, r4, k2, k3, &
                  ...
                  incflg, iravin, iflag, ialtd, ifxyz, idxyz)
  use ti_mod
  implicit none
  ...
  if (iflag .ne. 3) then
    ...
    if(ifxyz(4) .eq. 0) then
      ...
      if(ifxyz(5) .eq. 0) then
        ! check if the component is negative or positive
...
subroutine tornrg(crd, frc, dfr, ap, i1, i2, i3, i4, e, r1, r2, r3, r4, k2, k3, &
...
! dfr(12):   d(E)/d(x_com),d(E)/d(y_com) and d(E)/d(z_com) for position one
...
!
!              If nmrat(2,i) < 0, then
!                 (nmrcom(1,k) -> nmrcom(2,k)), k=-nmrat(2,i),-nmrat(16,i)
!              gives the atoms in the group whose center of mass is position 2.
! rmstot(4): The total masses for all the atoms defining the group at each
...
! by the derivatives in df, and add them into the force array.
    ...
    if (nmrat(iat, i) .lt. 0 .and. nmrat(iat+8, i) .lt. 0) then
...
! Formal arguments:
  ...
  double precision  crd(*)
  double precision  xcom(12) !3 * number of COM grps
  integer           nmrat(16, 1)
  integer           nmrcom(2, *)
  double precision  rmstot(4) ! for the 4 com groups
  integer           i
...
!         write(*,*)"Restrict by ", xtot, ytot, ztot, ifxyz(1), ifxyz(2), ifxyz(3)        
...
subroutine nmrsht(crd, nmrnum, nstep, nmrat, nmrcom, ishrtb, iwtstp, wtnrg, &
                  ...
                  ravein, dt, iout, iflag, ifxyz, idxyz, resttype)
...
! Local variables:
  ...
  double precision  dum_array(12)    ! maximum explicit size for dummy
  integer           i
  integer           iat1, iat2
  integer           iat(8)
  integer           iave
    ...
    do i = 1, nmrnum
      ...
      select case (resttypetmp)
        ...
        if (usecom) then 
          ...
          atmarr(1) = min(atmarr(1), atmarr(2))
          atmarr(2) = max(atmarr(3), atmarr(4))
          call at2res(atmarr(1), gbl_res_atms, nres, ires1, 1, iout)
          call at2res(atmarr(2), gbl_res_atms, nres, ires2, 1, iout)
        else
...
! Angle restraints:
        ...
        if (usecom) then 
          ...
          call at2res(atmarr(1), gbl_res_atms, nres, irs1, 1, iout)
          call at2res(atmarr(2), gbl_res_atms, nres, irs2, 1, iout)
        else 
...
! r**-6 averaged distance interaction restraints:
                ...
                igravt(i) .eq. 1) then
        call r6ave(crd, nmrat, nmrcom, rr, i, ifxyz(5*(i - 1) + 1), &
                   idxyz(3*(i - 1) +1)) 
...
! Regular distance restraints:
        ...
        call disnrg(crd, dum_array, dum_array, rr, nmrat(1, i), nmrat(2, i), &
                    ...
                    navint(1), iave, 0, 0, 1, ialtdis(i), ifxyz(5*(i - 1) + 1), &
                    idxyz(3*(i - 1) + 1))
...
! Angle restraints:
      ...
      if (usecom) then 
        ...
        call disnrg(xcom, dum_array, dum_array, rr1, 0, 3,                   &
                    dum, dum, dum, dum, dum, dum, dum, dum_array, dum_array, &
                    ipower(2), tauave(2), ravein(2), dt, 0, 0, 0, 0, 1,      &
                    ialtdis(i), ifxyz(5*(i - 1) + 1), idxyz(3*(i - 1) + 1))

        call disnrg(xcom, dum_array, dum_array, rr2, 3, 6,                   &
                    dum, dum, dum, dum, dum, dum, dum, dum_array, dum_array, &
                    ipower(2), tauave(2), ravein(2), dt, 0, 0, 0, 0, 1,      &
                    ialtdis(i), ifxyz(5*(i - 1) + 1), idxyz(3*(i - 1) + 1))
      else  
...
! Torsional restraints:
      ...
      if (usecom) then 
        ...
        call disnrg(xcom, dum_array, dum_array, rr2, 3, 6, &
                    ...
                    ipower(3), tauave(3), ravein(3), dt, 0, iave, 0, 0, 1, &
                    ialtdis(i), ifxyz(5*(i - 1) + 1), idxyz(3*(i - 1) + 1))
        call disnrg(xcom, dum_array, dum_array, rr3, 6, 9, &
                    dum, dum, dum, dum, dum, dum, dum, dum_array, dum_array, &
                    ipower(3), tauave(3), ravein(3), dt, 0, iave, 0, 0, 1, &
                    ialtdis(i), ifxyz(5*(i - 1) + 1), idxyz(3*(i - 1) + 1))
      else
...
subroutine r6ave(crd, nmrat, nmrcom, rave, indx, ifxyz, idxyz)
  ...
  if (iat1 .ge. 0) then
    ...
    do jp = -iat2, -nmrat(10, indx)
      do j = nmrcom(1, jp), nmrcom(2, jp)
        ...
        iat0(ipr) = i3
        jat0(ipr) = j3
        ! if we are using ifxyz option, then one of the x, y or z component 
        ! will be zeroed, else ifxyz(1),ifxyz(2),ifxyz(3) will be 1 so it 
        ! should not affect the regular average distance calculation 
        ! Maybe I should have a check here?
        !rij2 = xij(1, ipr)**2 + xij(2, ipr)**2 + xij(3, ipr)**2
        rij2 = (xij(1, ipr)*(ifxyz(1)))**2 + &
               (xij(2, ipr)*(ifxyz(2)))**2 + &
               (xij(3, ipr)*(ifxyz(3)))**2
        rm2 = 1.0/rij2
        rm6bar = rm6bar + rm2**3
...
! Case where iat1 < 0 and iat2 < 0 (both r**-6 ave. pos.):
    ...
    rm6bar = rm6bar/fsum
    rij = rm6bar**(- one6)
    ! ifxyz(4)=0 means only one component of the distance is zeroed
    ! ifxyz(4) equal to 1 then only the x component of the distance is calculated
    !          and we will pass that x component distance to outputfile
    !          2 is y component and 3 is z component
    if (ifxyz(4) .ne. 0) then 
      rij = rm6bar**(- one6)
      idxyz(ifxyz(4))= rij
    end if
...
subroutine r6drv(frc, df, ifxyz, idxyz)
...
! r6avec common block. Shared between this routine and r6drv:
  ...
  do ipr = 1, nsum
    ...
    j3 = jat0(ipr)
    fact2 = fact*rm8(ipr)
    ! ifxyz here is either 1 or 0 and if it it zero, its intend
    ! is to zero the x or y or z component 
    xh = fact2*xij(1, ipr)*ifxyz(1)
    yh = fact2*xij(2, ipr)*ifxyz(2)
    zh = fact2*xij(3, ipr)*ifxyz(3)
