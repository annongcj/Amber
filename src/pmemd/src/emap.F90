...
public   
    ...
    TYPE EMAPOBJECT           ! map object type
      ...
      double precision,pointer :: xatom(:)=> null(),yatom(:)=> null(),zatom(:)=> null()        !  pointer to atom coordinates
      ...
      double precision    :: avg,std,max,min   ! statistics
      double precision    :: alpha,beta,gamma   ! lattice angles (degrees)
      double precision    :: cx,cy,cz  !  map center in the map coordinate (vexols)
      double precision    :: dx,dy,dz  !  grid sizes (angstroms/vexol)
      double precision    :: ux,uy,uz           !  unit cell sizes (angstroms)
      double precision    :: ox,oy,oz           !  absolute coordinates of map center (angstroms)
      double precision    :: sx,sy,sz           !  parameters for reduced coordinates (angstroms)
    ...
    TYPE EMAPRIGID        ! rigid domain type
      ...
      double precision :: RESOLUTION        !  resolution of the map object
      ...
      double precision :: mass,winv,tcm         ! mass, invert
      ...
      double precision,dimension(3) :: prinv   ! inverse of principle inertia
      double precision,dimension(3) :: center         ! Position center coordinates 
      double precision,dimension(3) :: eular ! Eular angles:phi,theta,psi  
      double precision,dimension(4) :: quad   ! quarterions  
      double precision,dimension(3) :: v         ! velocities 
      double precision,dimension(3) :: omiga        ! angular velocities 
      double precision,dimension(3) :: angm        ! angular momentum 
      double precision,dimension(3) :: TRAN,TRANREF ! Translation vectors
      double precision,dimension(3,3) :: ROT,ROTREF ! rotational matrix
      double precision,dimension(3) :: force ! force on center of mass
      double precision,dimension(3) :: torq ! torque
    ...
    double precision EKMAP,MAPDT,XGAMMA,XAVG0,XAVG1,EMSIGMA,SCEMAP
...
!_________________________________________________________________
  ...
  implicit none
      ...
      character(len=256) atmask
      double precision fcons,resolution
...
!  search for namelist EMAP
      ...
      if(alloc_err /= 0 ) write(mdout,*)"unable to allocate EMRIGS"
      rewind UNIT
      DO I=1,NRIGID
        mapfile=''
        atmask=':*'
      ...
      if(alloc_err /= 0 ) write(mdout,*)"unable to allocate EMAPS"
      J=0
      DO I=1,NRIGID
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      SUBROUTINE PEMAP(dtmol,tempmol)
!_________________________________________________________________
...
!_________________________________________________________________
  ...
  use inpcrd_dat_mod, only: atm_crd,atm_frc
  use prmtop_dat_mod, only: natom,nres, &
           gbl_res_atms,gbl_labres,atm_igraph,atm_mass,atm_isymbl
      double precision dtmol,tempmol

      INTEGER I,J,nm,iat,i3,j3,idemp
      INTEGER alloc_err,ierr,UNITOUT,grids(6)
      double precision RESO,gridsize
      INTEGER, allocatable, dimension(:)::mask
      double precision, allocatable, dimension(:)::TMP_CRD,TMP_AMASS
!
      BORDER=6
      UNITOUT=6
...
#ifdef MPI
      if ( numtasks > 1 ) then
        call mpi_bcast(nemap,1,MPI_INTEGER,0,pmemd_comm,ierr)
        call mpi_bcast(nrigid,1,MPI_INTEGER,0,pmemd_comm,ierr)
        if ( .not. master ) then
          UNITOUT=-1
          if(allocated(EMRIGS))deallocate(EMRIGS)
          allocate(EMRIGS(NRIGID),stat=alloc_err)
        ...
        DO I=1,NRIGID
          if ( .not. master )call rigid_init(emrigs(i))
          call mpi_bcast(emrigs(i)%mapid,1,MPI_INTEGER,0,pmemd_comm,ierr)
          call mpi_bcast(emrigs(i)%rigid,1,MPI_INTEGER,0,pmemd_comm,ierr)
          call mpi_bcast(emrigs(i)%name,80,MPI_CHARACTER,0,pmemd_comm,ierr)
          call mpi_bcast(emrigs(i)%mapfit,80,MPI_CHARACTER,0,pmemd_comm,ierr)
          call mpi_bcast(emrigs(i)%atmask,256,MPI_CHARACTER,0,pmemd_comm,ierr)
          call mpi_bcast(emrigs(i)%fcons,1,MPI_DOUBLE_PRECISION,0,pmemd_comm,ierr)
          call mpi_bcast(emrigs(i)%resolution,1,MPI_DOUBLE_PRECISION,0,pmemd_comm,ierr)
          call mpi_bcast(emrigs(i)%movable,1,MPI_LOGICAL,0,pmemd_comm,ierr)
          call mpi_bcast(emrigs(i)%fitting,1,MPI_LOGICAL,0,pmemd_comm,ierr)
          call mpi_bcast(emrigs(i)%nminim,1,MPI_INTEGER,0,pmemd_comm,ierr)
          call mpi_bcast(emrigs(i)%grids,6,MPI_INTEGER,0,pmemd_comm,ierr)
        ENDDO
        DO I=1,NEMAP
          call mpi_bcast(emaps(i)%mapfile,80,MPI_CHARACTER,0,pmemd_comm,ierr)
        ENDDO
      end if
#endif
      ...
      DO I=1,NRIGID
        call atommask( natom, nres, 0, atm_igraph, atm_isymbl, &
               gbl_res_atms, gbl_labres, atm_crd, emrigs(i)%atmask, mask )
        nm=0
        DO J=1,NATOM
          if(mask(J)>0)nm=nm+1
        ENDDO
        ...
        if(alloc_err /= 0 ) write(mdout,*)"unable to allocate IDXATOM"
        nm=0
        DO J=1,NATOM
          if(mask(J)>0)then
            nm=nm+1
        ...
        ENDDO
        call rigid_prop(atm_mass,atm_crd,emrigs(i))
        if(len_trim(emrigs(i)%name)==0)then
          ...
          if(alloc_err /= 0 ) write(mdout,*)"unable to allocate TMP_CRD"
          if(allocated(TMP_AMASS))deallocate(TMP_AMASS)
          allocate(TMP_AMASS(NM),stat=alloc_err)
          if(alloc_err /= 0 ) write(mdout,*)"unable to allocate TMP_AMASS"
          DO IAT=1,NM
            J=emrigs(i)%idxatom(iat)
            tmp_amass(iat)=atm_mass(iat)
            i3=3*iat-2
            tmp_crd(i3)=atm_crd(1,iat)
            tmp_crd(i3+1)=atm_crd(2,iat)
            tmp_crd(i3+2)=atm_crd(3,iat)
          enddo
          IDEMP=emrigs(i)%mapid
          RESO=emrigs(i)%resolution
          grids=emrigs(i)%grids
          if(RESO<0.0D0)then
            gridsize=-reso
            if(unitout>0)then
              write(unitout,'("Boundary Map ",I4," is created from ",i6," constrained atoms")')IDEMP,nm
              write(unitout,'(" gridsize: ",F4.1," grids: ",6I6)') gridsize,grids
              if (emrigs(i)%fitting)write(unitout,'("WARNING: No fitting for Boundary Map!")')
              emrigs(i)%fitting=.false.
              endif
            CALL BOUNDARYMAP(IDEMP,gridsize,grids,nm,tmp_amass,tmp_crd,unitout)      
          else 
            if(unitout>0)then
...
!
      ...
      DO I=1,NRIGID
        ...
        if(dealloc_err /= 0 ) write(mdout,*)"unable to deallocate IDXATOM for RIGID ",I
      ENDDO
      if(allocated(EMRIGS))deallocate(EMRIGS)
      if(allocated(EMAPS))deallocate(EMAPS)
      return
...
!_________________________________________________________________
     use gbl_constants_mod, only: PI
      ...
      double precision dtx,gx,gy,gz,phi,psi,tht,dphi,dpsi,dtht
      double precision xl,yl,zl,enrig,pos0(6),pos(6),posi(6),gtol
      double precision p(7,6),engs(7),engi
...
!                    By Xiongwu Wu, wuxw@nhlbi.nih.gov
!_________________________________________________________________
      double precision amass(*),CRD(*)
      type(EMAPRIGID) :: rigobj

      INTEGER I,J,N,IA
      double precision AMASSI,X,Y,Z,XC,YC,ZC,TCM,TMASS
...
!                    By Xiongwu Wu, wuxw@nhlbi.nih.gov
!_________________________________________________________________
      double precision forcetmp(3),torqtmp(3),enemap
...
!
      ...
      INTEGER I
      double precision  ff,tt,scalf,scalt,ek
      double precision  DRF,DGT,phi,psi,tht,dphi,dpsi,dtht
...
!_______________________________________________________________________
    ...
    !
      ...
      double precision rot2quad(4),q1,q2,q3,q4,qsum
    !
...
!_______________________________________________________________________
    ...
    !
      implicit none
    !
      double precision U(3,3),PHI,PSI,THT
    !
      double precision sinF,cosF,sinY,cosY,sinS,cosS
    !
      sinF=sin(phi)
      cosF=cos(phi)
      sinY=sin(psi)
...
!_________________________________________________________________
    ...
    !
      implicit none
    !
      double precision PHI,PSI,THT
    !
      double precision a(3,3),daf(3,3),day(3,3),das(3,3)
      double precision sinf,cosf,siny,cosy,sins,coss
    !
      sinf=sin(phi)
      siny=sin(psi)
    ...
    !
      implicit none
    !
      double precision U(3,3),RN(3),PHIR
    !
      double precision rnorm,rx,ry,rz,cosp,cosp1,sinp
      INTEGER I,J
    !
      cosp=cos(phir)
      cosp1=1.0D0-cosp
...
!                    By Xiongwu Wu, wuxw@nhlbi.nih.gov
!_________________________________________________________________
  use prmtop_dat_mod, only: nres,gbl_res_atms,gbl_labres,atm_igraph
  use inpcrd_dat_mod, only: atm_crd
!
      INTEGER NATRIG,IDXATM(*)
      double precision  CX,CY,CZ,T(3),U(3,3)
!
...
!
      do ires = 1,nres
         do iatom = gbl_res_atms(ires), gbl_res_atms(ires+1)-1
           ...
           ENDDO
           IF(ATSKIP)CYCLE
           XN=atm_crd(1,ia)-T(1)-CX
           YN=atm_crd(2,ia)-T(2)-CY
           ZN=atm_crd(3,ia)-T(3)-CZ
           atm_crd(1,ia)=U(1,1)*XN+U(2,1)*YN+U(3,1)*ZN+CX
           atm_crd(2,ia)=U(1,2)*XN+U(2,2)*YN+U(3,2)*ZN+CY
           atm_crd(3,ia)=U(1,3)*XN+U(2,3)*YN+U(3,3)*ZN+CZ
         end do
      end do
      RETURN
...
!                    By Xiongwu Wu, wuxw@nhlbi.nih.gov
!_________________________________________________________________
      use bspline_mod,only:fill_bspline_0
!
      type(EMAPOBJECT) :: mapn,map
      INTEGER UNIT,IDEMP,OUTU
      double precision  T(3),U(3,3),RESO
!
      INTEGER*4 LX,LY,LZ,MODE,MNX,MNY,MNZ,NX,NY,NZ
      double precision CX,CY,CZ,DX,DY,DZ,ALPHA,BETA,GAMMA
      ...
      double precision CXN,CYN,CZN,DXN,DYN,DZN
!
      INTEGER I,J,K,IN,JN,KN,M1,M2,M3,alloc_err
      INTEGER IPT1,IPT2,IPT3,ITH1,ITH2,ITH3,MORDER
      double precision X,Y,Z,XN,YN,ZN,XM,YM,ZM
      double precision FR1,FR2,FR3,RHOI
      double precision W,BS1(8),BS2(8),BS3(8)
      double precision VAL0,VAL0A
!
      LX=MAP%LX
      LY=MAP%LY
      LZ=MAP%LZ
      ...
      if(alloc_err /= 0 ) write(mdout,*)"unable to allocate RDATA"
      MAPN%LX=LXN
      MAPN%LY=LYN
      MAPN%LZ=LZN
      MAPN%MX=MXN
...
!
      write(unit,'(A)') 'REMARK  Written by Amber 16, PMEMD, EMAP'
      do ires = 1,nres
         do iatom = gbl_res_atms(ires), gbl_res_atms(ires+1)-1
           ATSKIP=.TRUE.
           DO I=1,NATRIG
...
! print map information
      ...
      WRITE(OUTU,1150)trim(MACHST)             ! 54
      WRITE(OUTU,1160)(I,trim(NOTES(I)),I=1,NNOTE)   ! 57-256
      WRITE(OUTU,1210)NDATA                  ! DATA NUMBER
      !do iz=1,lz
      !  do iy=1,ly
      !    i=ly*(iy-1+lz*(iz-1))
          !WRITE(OUTU,1221)lx,iy,iz,(EMAPS(IDEMP)%RDATA(ix+I),ix=1,lx)           ! 
      !    WRITE(OUTU,1221)lx,iy,iz,(MAP%RDATA(ix+I),ix=1,lx)           ! 
      !  enddo
      !enddo
...
!
      ...
      double precision DXU,DYU,DZU,DXC,DYC,DZC,DDX,DDY,DDZ,DDX1,DDY1,DDZ1
      double precision ONE,TWO,COSA,COSB,COSG,SING,VUC
      double precision MAT11,MAT12,MAT13,MAT22,MAT23,MAT33
      double precision BG,XI,YI,ZI
...
! write data
      NDATA=LX*LY*LZ
      allocate(RDATA(NDATA),stat=alloc_err)
...
!     &    //"             Email: wuxw@nhlbi.nih.gov "
      ...
      if (alloc_err /= 0 ) then
        write(mdout,'( "Error:  Unable to allocate EMAP rdata !" )')
        write(mdout,'(" Work map size and background: ",3I6,F10.4)')LXC,LYC,LZC,BG
        call setup_alloc_error
      endif
      EMAPS(IDEMP)%RDATA(:)=BG
      DO I=1,NDATA
        ...
        EMAPS(IDEMP)%RDATA(IXYZ)=EMAPS(IDEMP)%RDATA(IXYZ)+DDX*DDY*DDZ*RDATAI
        IXYZ=JX+LXC*((JY1-1+(JZ-1)*LYC))
        EMAPS(IDEMP)%RDATA(IXYZ)=EMAPS(IDEMP)%RDATA(IXYZ)+DDX*DDY1*DDZ*RDATAI
        IXYZ=JX+LXC*((JY-1+(JZ1-1)*LYC))
        EMAPS(IDEMP)%RDATA(IXYZ)=EMAPS(IDEMP)%RDATA(IXYZ)+DDX*DDY*DDZ1*RDATAI
        IXYZ=JX+LXC*((JY1-1+(JZ1-1)*LYC))
        EMAPS(IDEMP)%RDATA(IXYZ)=EMAPS(IDEMP)%RDATA(IXYZ)+DDX*DDY1*DDZ1*RDATAI
        IXYZ=JX1+LXC*((JY-1+(JZ-1)*LYC))
        EMAPS(IDEMP)%RDATA(IXYZ)=EMAPS(IDEMP)%RDATA(IXYZ)+DDX1*DDY*DDZ*RDATAI
        IXYZ=JX1+LXC*((JY1-1+(JZ-1)*LYC))
        EMAPS(IDEMP)%RDATA(IXYZ)=EMAPS(IDEMP)%RDATA(IXYZ)+DDX1*DDY1*DDZ*RDATAI
        IXYZ=JX1+LXC*((JY-1+(JZ1-1)*LYC))
        EMAPS(IDEMP)%RDATA(IXYZ)=EMAPS(IDEMP)%RDATA(IXYZ)+DDX1*DDY*DDZ1*RDATAI
        IXYZ=JX1+LXC*((JY1-1+(JZ1-1)*LYC))
        EMAPS(IDEMP)%RDATA(IXYZ)=EMAPS(IDEMP)%RDATA(IXYZ)+DDX1*DDY1*DDZ1*RDATAI
...
! print map information
      ...
      !    i=ly*(iy-1+lz*(iz-1))
          !WRITE(OUTU,1221)lx,iy,iz,(EMAPS(IDEMP)%RDATA(ix+I),ix=1,lx)           ! 
      !    WRITE(OUTU,1221)lx,iy,iz,(RDATA(ix+I),EMAPS(IDEMP)%RDATA(ix+I),ix=1,lx)           ! 
      !  enddo
      !enddo
...
!
      ...
      double precision XIN,YIN,ZIN,AMASSI,RHOI,DG,WIN
...
120   CONTINUE
      ...
      if(alloc_err /= 0 ) write(mdout,*)"unable to allocate CRD !"
      if(allocated(tmp_amass))deallocate(tmp_amass)
      allocate(tmp_amass(NAT),stat=alloc_err)
      if(alloc_err /= 0 ) write(mdout,*)"unable to allocate AMASS !"
      CLOSE(UNIT)
      OPEN(UNIT,FILE=FNAME,ACCESS='SEQUENTIAL',STATUS='OLD')
      IAT=0
...
!      NNOTE=3
      NOTES(1)="          " &
         //" This map is created with Amber "
      NOTES(2)="          " &
         //" Report questions to Dr. Xiongwu Wu  "
      NOTES(3)="          " &
        //"             Email: wuxw@nhlbi.nih.gov "
...
1220  FORMAT(" ----------------------- END OF EMAP IMAGE ",I4, &
      ...
      SUBROUTINE BOUNDARYMAP(IDEMP,DGRID,GRIDS,NATC,AMASS,CRD,OUTU)
        !_________________________________________________________________
        !  Blur input coordinates to a map object
        !                    By Xiongwu Wu, wuxw@nhlbi.nih.gov
        !_________________________________________________________________
              use bspline_mod,only:fill_bspline_0
        !
              INTEGER IDEMP,NATC,OUTU,GRIDS(6)
              double precision AMASS(*)
              double precision CRD(*)
        ...
        !
              INTEGER*4 LX,LY,LZ,MODE,MNX,MNY,MNZ,NX,NY,NZ
              INTEGER SX,SY,SZ,HX,HY,HZ,HX2,HY2,HZ2,JX,JY,JZ,JXYZ,JXYZ0
              REAL*4 XC,YC,ZC,ALPHA,BETA,GAMMA
              INTEGER*4 MAPC,MAPR,MAPS
              INTEGER*4 ISPG,NSYMBT,LSKFLG,NNOTE
              REAL*4 SKWMAT(9),SKWTRN(3),EXTRA(15)
              ...
              INTEGER ISEQ,IAT
        !
              INTEGER I,J,K,IN,JN,KN,M1,M2,M3,NDATA,IXYZ
              INTEGER IPT1,IPT2,IPT3,ITH1,ITH2,ITH3
              double precision XMIN,YMIN,ZMIN,XMAX,YMAX,ZMAX
              REAL*4 dens1,dens2
              INTEGER alloc_err
        ...
        !
              ...
              YC=YC/NATC
              ZC=ZC/NATC
              if((xmax-xmin)>grids(1)*dgrid) &
              write(mdout,'(A,2F10.2)')"WARNING: Boundary map is too small in x direction!",(xmax-xmin),grids(1)*dgrid
              if((ymax-ymin)>grids(2)*dgrid) &
              write(mdout,'(A,2F10.2)')"WARNING: Boundary map is too small in y direction!",(ymax-ymin),grids(2)*dgrid
              if((zmax-zmin)>grids(3)*dgrid) &
              write(mdout,'(A,2F10.2)')"WARNING: Boundary map is too small in z direction!",(zmax-zmin),grids(3)*dgrid
              HX=(grids(1)/2+1)
              HY=(grids(2)/2+1)
              HZ=(grids(3)/2+1)
              HX2=HX*HX
              ...
              LY=HY*2+3
              LZ=HZ*2+3
              MNX=nint((XC-LX*DGRID/2.0D0)/DGRID)
              MNY=nint((YC-LY*DGRID/2.0D0)/DGRID)
              MNZ=nint((ZC-LZ*DGRID/2.0D0)/DGRID)
              NDATA=LX*LY*LZ
              if(associated(EMAPS(IDEMP)%RDATA))deallocate(EMAPS(IDEMP)%RDATA)
              allocate(EMAPS(IDEMP)%RDATA(NDATA),stat=alloc_err)
              if(alloc_err /= 0 ) write(mdout,*)"unable to allocate RDATA"
              ...
              dens1=100.0d0
              dens2=20.0d0
        !   generate map high boundary densities
              IXYZ=0
              DO K=1,LZ
                DO J=1,LY
                  DO I=1,LX
                    IXYZ=IXYZ+1
                    ! outer layer 
                    if((i-1)*(i-lx)*(j-1)*(j-ly)*(k-1)*(k-lz)==0)then
                      RHO(IXYZ)=dens1
                    else if((i-2)*(i-lx+1)*(j-2)*(j-ly+1)*(k-2)*(k-lz+1)==0)then
                      RHO(IXYZ)=dens2
                    else
                      ! outside of grids defined shape
                      ! hyper ellipse: grids(6)=(Dx,Dy,Dz,1-Sx,1-Sy,1-Sz)
                      ! Sx,Sy,Sz:1-ellipse,0: rectangular
                      JX=I-HX-2
                      JY=J-HY-2
                      JZ=K-HZ-2
                      JXYZ=SX*JX*JX*HY2*HZ2+SY*JY*JY*HZ2*HX2+SZ*JZ*JZ*HX2*HY2
                      IF(JXYZ>JXYZ0)RHO(IXYZ)=dens2
                    endif
                  ENDDO
                ENDDO
              ENDDO  
        !     assign map object
              ...
              EMAPS(IDEMP)%NY=LY
              EMAPS(IDEMP)%NZ=LZ
              EMAPS(IDEMP)%DX=DGRID
              EMAPS(IDEMP)%DY=DGRID
              EMAPS(IDEMP)%DZ=DGRID
              EMAPS(IDEMP)%ALPHA=90.0D0
              EMAPS(IDEMP)%BETA=90.0D0
              EMAPS(IDEMP)%GAMMA=90.0D0
        !      MAPC=1
        ...
        !      NNOTE=3
              NOTES(1)="          " &
                 //" This boundary map is created with  Amber "
              NOTES(2)="          " &
                 //" Report questions to Dr. Xiongwu Wu  "
              NOTES(3)="          " &
                //"             Email: wuxw@nhlbi.nih.gov "
        ...
        ! print map information
              ...
              WRITE(OUTU,1020)MODE                   ! 4
              WRITE(OUTU,1030)MNX,MNY,MNZ            ! 5,6,7
              WRITE(OUTU,1050)LX*DGRID,LY*DGRID,LZ*DGRID            ! 5,6,7
              WRITE(OUTU,1080)EMAPS(IDEMP)%MIN,EMAPS(IDEMP)%MAX,EMAPS(IDEMP)%AVG,EMAPS(IDEMP)%STD       ! 20,21,22,55
        !      WRITE(OUTU,1090)ISPG,NSYMBT,LSKFLG,NNOTE     ! 23,24,25,56
        !      WRITE(OUTU,1110)SKWMAT                 ! 26-34
        !      WRITE(OUTU,1120)SKWTRN                 ! 35-37
      ...
      !  do j=1,ly
      !    ixyz=lx*(j-1+ly*(k-1))
          !WRITE(OUTU,1221)lx,j,k,(EMAPS(IDEMP)%RDATA(ixyz+I),i=1,lx)           ! 
      !    WRITE(OUTU,1221)lx,j,k,(RHO(ixyz+I),i=1,lx)           ! 
      !  enddo
      !enddo
              ENDIF
        1000  FORMAT(" ------------------EMAP ID ",I4," CREATED ---------------" )
...
!
      INTEGER NDATA,I
      real(kind=4) RHOI
      double precision AMAX,AMIN,AMEAN,ARMS
!
...
!                    By Xiongwu Wu, wuxw@nhlbi.nih.gov
!_________________________________________________________________
      double precision ENEMAP,AMASS(*),X(*),F(*)
      INTEGER NATOM,I
      double precision ENRIG,SCRIG,TMASS
      ENEMAP=0.0D0
      SCEMAP=0.0D0
      TMASS=0.0D0
...
!_________________________________________________________________
!
      use bspline_mod,only:fill_bspline_1
!
#ifdef MPI
   include 'mpif.h'
#endif
      INTEGER NATOM
      double precision ENRIG,SCRIG,AMASS(*),X(*)
      type(EMAPRIGID) :: rigobj
      type(EMAPOBJECT) :: mapobj
      real(kind=4),pointer:: rho(:)=> null()        !   real data
      integer,pointer:: idx(:)=> null()        !   real data
      INTEGER NATC,MNX,MNY,MNZ,LX,LY,LZ
      double precision TR(3),U(3,3),PGUID,RIG_F(3),RIG_T(3)
      double precision RAVG,RHO0,CX,CY,CZ,GX,GY,GZ
      double precision BS1(8),BS2(8),BS3(8)
      double precision W,DBS1(8),DBS2(8),DBS3(8)
      double precision XI,YI,ZI,XJ,YJ,ZJ
      double precision AMASSI,RHOI,ENRIGM
      double precision CFACT,CFACTX,CFACTY,CFACTZ,FR1,FR2,FR3
      double precision VAL0,VAL1,VAL2,VAL3,VAL0A,VAL1A,VAL2A,VAL3A
      INTEGER ID,IAT,IATX,IATY,IATZ,I,J,K,M1,M2,M3
      INTEGER ITH1,ITH2,ITH3,IPT1,IPT2,IPT3,IERR
      double precision TEMP1(10),TEMP2(10)
!
      ...
      RHO=>MAPOBJ%RDATA
      RHO0=0.0d0
      if(rigobj%resolution>0.0d0)RHO0=mapobj%AVG
...
!  Calculate selected atoms
      DO ID=1,NATC
        IAT=IDX(ID)
#ifdef MPI
            if (gbl_atm_owner_map(iat) .ne. mytaskid) cycle
#endif
        IATX=3*IAT-2
        IATY=IATX+1
        IATZ=IATY+1
...
!
      INTEGER NATOM
      double precision ENRIG,SCRIG,AMASS(*),X(*),F(*)
      type(EMAPRIGID) :: rigobj
      type(EMAPOBJECT) :: mapobj
      real(kind=4),pointer:: rho(:)=> null()        !   real data
      integer,pointer:: idx(:)=> null()        !   real data
      ...
      double precision BS1(8),BS2(8),BS3(8)
      double precision W,DBS1(8),DBS2(8),DBS3(8)
      double precision XI,YI,ZI,XJ,YJ,ZJ,XC,YC,ZC
      double precision AMASSI,RHOI,ENRIGM,F1,F2,F3,FX,FY,FZ
      double precision CFACT,CFACTX,CFACTY,CFACTZ,FR1,FR2,FR3,T2
      double precision rfx,rfy,rfz,rtx,rty,rtz
      double precision VAL0,VAL1,VAL2,VAL3,VAL0A,VAL1A,VAL2A,VAL3A
      INTEGER ID,IAT,IATX,IATY,IATZ,I,J,K,M1,M2,M3
      INTEGER ITH1,ITH2,ITH3,IPT1,IPT2,IPT3,IERR
      double precision TEMP1(10),TEMP2(10)
...
# ifdef USE_MPI_IN_PLACE
          call mpi_allreduce(MPI_IN_PLACE,temp1,7,MPI_DOUBLE_PRECISION,MPI_SUM,pmemd_comm,ierr)
          SCRIG=TEMP1(1)
          RFX=TEMP1(2)
          RFY=TEMP1(3)
          RFZ=TEMP1(4)
...
!
  use inpcrd_dat_mod, only: atm_crd
  use prmtop_dat_mod, only: natom,atm_mass
      double precision rigeng,rigengi,RIGSCI
...
!
      integer i
       ...
       CALL RIGIDENG(NATOM,RIGENGI,RIGSCI,atm_mass,atm_crd,RIGMIN)
...
!
  ...
  use prmtop_dat_mod, only: natom,atm_mass
      ...
      double precision rigengd,rigengi,rigsci
      double precision pos(6),dpos(6)
!
      integer i
      double precision a(3,3),daf(3,3),day(3,3),das(3,3),b(3),t(3),c(3)
       ...
       CALL RIGIDFORCE(NATOM,RIGENGI,RIGSCI,atm_mass,atm_crd,atm_frc,RIGMIN)
...
!______________________________________________
      ...
      double precision simtry,fac,p(mp,np),psum(np),y(mp),funk
      PARAMETER (NMAX=20)  
      EXTERNAL funk  
!    USES funk  
      INTEGER j  
