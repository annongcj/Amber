...
use nfe_lib_mod, only : SL => STRING_LENGTH, ABMD_MONITOR_UNIT, ABMD_CV_UNIT, &
...
type(umbrella_t), private, save :: wt_umbrella ! master only
!-----------------------------------------------------------------------------
#ifndef MPI
double precision, private, save :: instantaneous(MAX_NUMBER_OF_COLVARS) ! master only
#else
double precision, private, save :: instantaneous(MAX_NUMBER_OF_COLVARS + 1)
#endif /* MPI */
!-----------------------------------------------------------------------------
character(len = SL), private, save :: monitor_file = DEFAULT_MONITOR_FILE! master only
...
!-----------------------------------------------------------------------------
character(len = SL), private, save :: snapshots_basename = DEFAULT_SNAPSHOTS_BASENAME ! master only
integer,             private, save :: snapshots_freq = -1 ! master only
!-----------------------------------------------------------------------------
character(len = SL), private, save :: umbrella_file = DEFAULT_UMBRELLA_FILE ! master only
character(len = SL), private, save :: wt_umbrella_file = DEFAULT_WT_UMBRELLA_FILE ! master only
!-----------------------------------------------------------------------------
double precision, private, save :: timescale = 1.0 ! master only
!-----------------------------------------------------------------------------
integer, public, save :: mdstep ! = runmd.f::nstep + 1
!-----------------------------------------------------------------------------
integer, private, save :: nhills ! not zeroed on exchange
!-----------------------------------------------------------------------------
! Modified by M Moradi
! Selection algorithm
integer,                    public,  save :: selection_freq = 0
double precision,           private, save :: selection_constant = 1.0
double precision,           private, save :: selection_epsilon = 0.0
type(random_state),         private, save :: nfe_abmd_gen
! Well-tempered ABMD
double precision,           private, save :: pseudo = 0.0
...
double precision, private, allocatable, save :: all_hills(:)

! Modified by M Moradi
! Selection algorithm
double precision, private, allocatable, save :: all_x(:)
double precision, private, allocatable, save :: all_v(:)
double precision, private, allocatable, save :: all_w(:)
double precision, private, allocatable, save :: all_box(:)
integer, private, allocatable, save :: all_n(:)
double precision, private, save :: my_w
integer, private, save :: my_n, n_tot
character(len = SL), private, save :: MDOUT_FILE
! Moradi end

!
! rem-specific, not used here in PMEMD
!

! rem_* arrays live on pmemd masters; indexed by master_rank
...
character(SL),private,save :: LOG_FILE
...
subroutine on_delta(o_master_rank, need_U_xx, U_mm, U_mo, U_om, U_oo)
   ...
   double precision :: o_instantaneous(MAX_NUMBER_OF_COLVARS), U_o(2), U_m(2)
   ...
   nfe_assert(multipmemd_rem().eq.1)
   nfe_assert(mytaskid.eq.0) ! master
   nfe_assert(pmemd_master_comm.ne.mpi_comm_null)

   ! exchange instantaneous(:) with the partner
   call mpi_sendrecv &
      (instantaneous, ncolvars, MPI_DOUBLE_PRECISION, o_master_rank, mdstep, &
       o_instantaneous, ncolvars, MPI_DOUBLE_PRECISION, o_master_rank, mdstep, &
   ...
   call mpi_sendrecv &
      (U_m, 2, MPI_DOUBLE_PRECISION, o_master_rank, mdstep, &
       U_o, 2, MPI_DOUBLE_PRECISION, o_master_rank, mdstep, &
       pmemd_master_comm, MPI_STATUS_IGNORE, error)
   nfe_assert(error.eq.0)
...
subroutine on_exchange(o_master_rank)
   ...
   exchange_partner = o_master_rank
...
subroutine on_pmemd_init(mdin_unit, amass)
   ...
   use random_mod, only : amrset_gen
   ...
   monitor_freq = min(monitor_freq, pmemd_nstlim())
   monitor_freq = max(1, monitor_freq)

! Modified by M Moradi and F Pan
! Selection algorithm
#ifdef MPI
...
1  continue ! done with parsing
...
#ifdef MPI
   call mpi_bcast(imode, 1, MPI_INTEGER, 0, pmemd_comm, error)
   ...
   call mpi_bcast(monitor_freq, 1, MPI_INTEGER, 0, pmemd_comm, error)
   nfe_assert(error.eq.0)
#endif /* MPI */
...
#ifdef MPI
   if (multipmemd_numgroup().gt.1) then
   call mpi_bcast(selection_freq, 1, MPI_INTEGER, 0, pmemd_comm, error)
   nfe_assert(error.eq.0)

#ifdef CUDA 
   if (selection_freq.gt.0 .and. pmemd_ntb().gt.1) then 
   write(unit = ERR_UNIT, fmt = '(/a,a/)') NFE_ERROR,'Selection algorithm &
         cannot run with ntb > 1 for CUDA, use CPU or constant volume instead!'
   call terminate()
   end if
#endif

   MDOUT_FILE = pmemd_mdout_name()
   call mpi_bcast(MDOUT_FILE, len(MDOUT_FILE), MPI_CHARACTER, 0, pmemd_comm, error)
   nfe_assert(error.eq.0)
   end if
#endif /* MPI */
! Moradi end
...
! Selection algorithm
      ...
      if (multipmemd_numgroup().gt.1) then
         if (selection_freq.gt.0) then
            my_w = ZERO
            allocate(all_x(3*pmemd_natoms()*multipmemd_numgroup()), &
               stat = error)
            allocate(all_v(3*pmemd_natoms()*multipmemd_numgroup()), &
               stat = error)
            if (pmemd_ntb().ne.0) &
               allocate(all_box(3*multipmemd_numgroup()), &
                  stat = error)
            allocate(all_w(multipmemd_numgroup()), &
               stat = error)
            allocate(all_n(multipmemd_numgroup()), &
               stat = error)
            call amrset_gen(nfe_abmd_gen, ig+master_rank)
...
#ifdef MPI
      if (multipmemd_numgroup().gt.1) then
         allocate(all_hills((ncolvars + 1)*multipmemd_numgroup()), &
            stat = error)
! Modified by M Moradi
! Selection algorithm
#ifdef MPI
...
! Moradi end
         ...
         if (multipmemd_rem().eq.0) then
            ...
            call umbrella_bcast(umbrella, pmemd_master_comm, 0)

            call mpi_bcast(timescale, 1, &
               MPI_DOUBLE_PRECISION, 0, pmemd_master_comm, error)
            nfe_assert(error.eq.0)
         end if ! .not.REM
      end if ! ng.gt.1
#endif /* MPI */
   ...
   call flush_UNIT(MONITOR_UNIT, monitor_file)

   ! print summary & return
#ifdef MPI
   LOG_UNIT = OUT_UNIT  ! write to MDOUT
#endif
   LOG_FILE = pmemd_mdout_name()
   ...
   monitor_freq*pmemd_timestep(), 4)//',a)') NFE_INFO, &
      'monitor_freq = ', monitor_freq, ' (', &
      monitor_freq*pmemd_timestep(), ' ps)'

   if (imode.eq.MODE_ANALYSIS) &
      goto 3

#  ifdef MPI
   if (multipmemd_numgroup().gt.1.and.multipmemd_rem().eq.0) then
      if (master_rank.gt.0) &
...
#  endif /* MPI */
   ...
   if (snapshots_freq.gt.0) then
      ...
      write (unit = LOG_UNIT, &
      fmt = '(a,a,'//pfmt(snapshots_freq)//',a,'//pfmt(snapshots_freq*pmemd_timestep(), 4)//',a)')&
          NFE_INFO, 'snapshots_freq = ', snapshots_freq, ' (', &
         snapshots_freq*pmemd_timestep(), ' ps)'
   end if
...
! Selection algorithm
   ...
   if (multipmemd_numgroup().gt.1) then
     if (selection_freq.gt.0) then
      ...
      write (unit = LOG_UNIT, &
      fmt = '(a,a,'//pfmt(selection_freq)//',a,'//pfmt(selection_freq*pmemd_timestep(), 4)//',a)')&
          NFE_INFO, 'selection_freq = ', selection_freq, ' (', &
         selection_freq*pmemd_timestep(), ' ps)'
      write (unit = LOG_UNIT, &
...
! Moradi end
   ...
   call flush_UNIT(LOG_UNIT, LOG_FILE)
...
subroutine rem_checks()
   ...
   if (pmemd_master_comm.eq.mpi_comm_null) &
      return
   
   nfe_assert(master_size.gt.0)
   nfe_assert(master_rank.lt.master_size)

   ! For H-REMD and Multi-REMD, imode and ncolvar can vary among replicas   
   if (multipmemd_rem().eq.3 .or. multipmemd_rem().eq.-1) &
      return

   allocate(int_recv(master_size), stat = error)
   if (error.ne.0) &
      NFE_OUT_OF_MEMORY

   call mpi_allgather(imode, 1, MPI_INTEGER, int_recv, 1, MPI_INTEGER, &
                      pmemd_master_comm, error)
   nfe_assert(error.eq.0)
...
! Moradi end
   ...
   if (imode.eq.MODE_ANALYSIS) then
      if (nfe_real_mdstep.and.mod(mdstep, monitor_freq).eq.0) then
         ...
         write (unit = MONITOR_UNIT, fmt = monitor_fmt) &
            pmemd_mdtime(), instantaneous(1:ncolvars)
         call flush_UNIT(MONITOR_UNIT, monitor_file)
         NFE_MASTER_ONLY_END
      end if
      goto 1
   ...
   if (nfe_real_mdstep) then
      ...
      if (mod(mdstep, monitor_freq).eq.0) then
         if (imode.eq.MODE_FLOODING) then
            write (unit = MONITOR_UNIT, fmt = monitor_fmt) &
               pmemd_mdtime(), instantaneous(1:ncolvars), u_value
         else
            write (unit = MONITOR_UNIT, fmt = monitor_fmt) &
               pmemd_mdtime(), instantaneous(1:ncolvars)
         end if
...
! Driven ABMD (based on Moradi and Tajkhorshid, JPCL(2013) 4:1882)
         if (drivenw.ne.0) then
            if (drivenw*wdriven-drivenu*udriven.gt.driven_cutoff) then
               alt = alt * &
                exp(-(drivenw*wdriven-drivenu*udriven)/(kB*pmemd_temp0()))
            else
               alt = alt * &
                exp(-driven_cutoff/(kB*pmemd_temp0()))
            end if
         end if ! drivenw.ne.0
! Moradi end         
#ifdef MPI
         if (multipmemd_numgroup().gt.1) then
            if (multipmemd_rem().eq.0) then
               ...
               call mpi_allgather(instantaneous, ncolvars + 1, &
                  ...
                  MPI_DOUBLE_PRECISION, pmemd_master_comm, error)
               nfe_assert(error.eq.0)
            end if

            if (multipmemd_rem().eq.0) then
               do n = 0, multipmemd_numgroup() - 1
                  call umbrella_hill(umbrella, &
                     all_hills(n*(ncolvars + 1) + 1:), &
                     all_hills((n + 1)*(ncolvars + 1)))
               end do
            else if (multipmemd_rem().eq.6) then
               do n = 0, multipmemd_numgroup() - 1
                  if (n.eq.master_rank) then
                     call umbrella_hill(umbrella, &
                        all_hills(n*(ncolvars + 1) + 1:), &
                        all_hills((n + 1)*(ncolvars + 1)))
                  else
            ...
            call umbrella_hill(umbrella, instantaneous, alt)
#ifdef MPI
         end if ! multipmemd_numgroup().gt.1
#endif /* MPI */
         nhills = nhills + 1
      end if
...
subroutine on_mdstep(x, v)
   ...
   use nfe_lib_mod
   use nfe_colvar_mod
   use random_mod, only : amrand_gen
   use parallel_dat_mod
   use pbc_mod, only : pbc_box,pbc_alpha,pbc_beta,pbc_gamma,init_pbc
   ...
   integer :: error
   double precision :: my_rand, w_norm, my_bias, my_laplacian, selection_criterion
   integer :: my_n, n_tot
   integer :: n
   if (((imode.eq.MODE_FLOODING).or.(imode.eq.MODE_UMBRELLA)).and.(selection_freq.gt.0) &
      .and.(multipmemd_numgroup().gt.1)) then
      ...
      NFE_MASTER_ONLY_BEGIN
        ...
        if (mdstep.gt.1.and.mod(mdstep, selection_freq).eq.0) then
          if (multipmemd_numgroup().gt.1) then
            ...
            call mpi_allgather(x, 3*pmemd_natoms(), &
               MPI_DOUBLE_PRECISION, all_x, 3*pmemd_natoms(), &
               MPI_DOUBLE_PRECISION, pmemd_master_comm,error)
            nfe_assert(error.eq.0)
            ! get all v
            nfe_assert(allocated(all_v))
            call mpi_allgather(v, 3*pmemd_natoms(), &
               MPI_DOUBLE_PRECISION, all_v, 3*pmemd_natoms(), &
               MPI_DOUBLE_PRECISION, pmemd_master_comm,error)
            nfe_assert(error.eq.0)
            if (pmemd_ntb().ne.0) then
               nfe_assert(allocated(all_box))
               call mpi_allgather(pbc_box, 3, &
                  MPI_DOUBLE_PRECISION, all_box, 3, &
                  MPI_DOUBLE_PRECISION, pmemd_master_comm,error)
               nfe_assert(error.eq.0)
            end if ! pmemd_ntb().ne.0
            my_w = exp(my_w)
            if (my_w > 1.0d+20) &
            ...
            call mpi_allgather(my_w, 1, &
               ...
               MPI_DOUBLE_PRECISION, pmemd_master_comm,error)
            nfe_assert(error.eq.0)
            call mpi_barrier(pmemd_master_comm,error)
            allocate(all_n(multipmemd_numgroup()), stat = error)
            if (master_rank.eq.0) then
               ...
               if (abs(w_norm).gt.0) then
                  ...
                  do n = 1, multipmemd_numgroup()
                    my_n = int(my_rand+multipmemd_numgroup()*sum(all_w(1:n))/w_norm)
                      selection_criterion = selection_criterion + (all_w(n)/w_norm) &
                                        * log(multipmemd_numgroup()*all_w(n)/w_norm)
                    write (unit= LOG_UNIT , fmt =&
                      '(a,a,'//pfmt(n)//',a,'//pfmt(all_w(n),6)//',a,'&
                          //pfmt(w_norm,6)//',a,'//pfmt(all_w(n)/w_norm,3)//',a,'//pfmt(my_n-n_tot)//',a)')&
                  ...
                  if (selection_epsilon.le.ONE) then
                     nfe_assert(selection_criterion.ge.-TINY)
                     if (abs(selection_criterion).lt.TINY) &
                         selection_criterion = ZERO
                     if (selection_criterion.ge.selection_epsilon*log(dble(multipmemd_numgroup()))) then
                         write (unit= LOG_UNIT , fmt =&
                            '(a,a,'//pfmt(selection_criterion,6)//',a,'&
                            //pfmt(selection_epsilon*log(dble(multipmemd_numgroup())),6)//')')&
                            NFE_INFO,'Selection entropy ',selection_criterion,&
                            ' is greater than threshold ', &
                            selection_epsilon*log(dble(multipmemd_numgroup()))
                     else
                         write (unit= LOG_UNIT , fmt =&
                            '(a/,a,a,'//pfmt(selection_criterion,6)//',a,'&
                            //pfmt(selection_epsilon*log(dble(multipmemd_numgroup())),6)//'/,a,a/,a)')&
            ...
            call mpi_bcast(selection_freq, 1, MPI_INTEGER, 0, pmemd_master_comm,error)
            nfe_assert(error.eq.0)
            call mpi_bcast(all_n, multipmemd_numgroup(), &
               MPI_INTEGER, 0, pmemd_master_comm,error)
            nfe_assert(error.eq.0)
            call mpi_barrier(pmemd_master_comm,error)
            my_w = ZERO
            if (master_rank+1.ne.all_n(master_rank+1)) then
              x(1:3*pmemd_natoms()) = &
                 all_x((all_n(master_rank+1)-1)*3*pmemd_natoms()+1:all_n(master_rank+1)*3*pmemd_natoms())
              v(1:3*pmemd_natoms()) = &
                 all_v((all_n(master_rank+1)-1)*3*pmemd_natoms()+1:all_n(master_rank+1)*3*pmemd_natoms())
              if (pmemd_ntb().ne.0) &
                 pbc_box(1:3) = &
                    all_box((all_n(master_rank+1)-1)*3+1:all_n(master_rank+1)*3)
              write (unit= LOG_UNIT , fmt ='(a,a,'//pfmt(master_rank+1)//',a,'//pfmt(all_n(master_rank+1))//'/)')&
                 NFE_INFO,'Selection resampling : new ',master_rank+1,' comes from ',all_n(master_rank+1)
            end if
      ...
      call mpi_bcast(selection_freq, 1, MPI_INTEGER, 0, pmemd_comm, error)
      nfe_assert(error.eq.0)
      if (mdstep.gt.1.and.mod(mdstep, abs(selection_freq)).eq.0) then

         call mpi_bcast(x(1:3*pmemd_natoms()), 3*pmemd_natoms(), MPI_DOUBLE_PRECISION, 0, pmemd_comm, error)
         nfe_assert(error.eq.0)
         call mpi_bcast(v(1:3*pmemd_natoms()), 3*pmemd_natoms(), MPI_DOUBLE_PRECISION, 0, pmemd_comm, error)
         nfe_assert(error.eq.0)

         ! The NPT run has problems right now with CUDA
         if (pmemd_ntb().gt.1) then
            call mpi_bcast(pbc_box(1:3), 3, MPI_DOUBLE_PRECISION, 0, pmemd_comm, error)
            nfe_assert(error.eq.0)
            open(unit=OUT_UNIT, file=nullfile, status='old')
            call init_pbc(pbc_box(1),pbc_box(2),pbc_box(3),pbc_alpha,pbc_beta,pbc_gamma,vdw_cutoff + skinnb)
            close(unit=OUT_UNIT)
            NFE_MASTER_ONLY_BEGIN
...
! Moradi end
!><><><><><><><><><><><><><><><>< R E M D ><><><><><><><><><><><><><><><><><><
! The following subroutines are old ones, not used now
#ifdef MPI

!
! populates rem_* arrays (indexed by master_rank)
!
...
subroutine rem_postinit()
   ...
   do i = 0, ng
      ...
      if (imode.eq.MODE_UMBRELLA.or.imode.eq.MODE_FLOODING) then
         ...
         call umbrella_bcast(rem_umbrellas(i), pmemd_master_comm, i)
...
subroutine finalize_umbrellas
   ...
   use nfe_lib_mod, only : umbrella_fini, multipmemd_numgroup
   use parallel_dat_mod
