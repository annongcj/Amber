#!/bin/python
#AUTOGENERATE CODE to create get_nb_energy_ti_AUTO.i

pair =  ['linear_V0', 'linear_V1' ,'sc_common_V0', 'sc_common_V1', 'sc_sc_V0', 'sc_sc_V1' ]
cut = ['cut1','cut2']
etype = ['mefv','efv','fv','f']

f = open('get_nb_energy_ti_AUTO.i', 'w')
f.write('! NOTE: This code is autogenerated by ti_code1.py, do not edit this file\n')
...
for i in xrange(0,len(pair)):
    f.write("""
        if (ti_ee_eval_cnt(%(index)d) + ti_full_eval_cnt(%(index)d) .gt. 0) then         
          ee_eval_cnt = ti_ee_eval_cnt(%(index)d)
          full_eval_cnt = ti_full_eval_cnt(%(index)d)
          ...
          if(use_pme .eq. 0) then
            if (cutoffs_equal) then
              if (ifmbar_lcl .ne. 0 .and. do_mbar) then            
                  call pairs_calc_ti_%(type)s_cut1_mefv_vac(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)          
              else
                  call pairs_calc_ti_%(type)s_cut1_efv_vac(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)               
              end if
            else
              if (ifmbar_lcl .ne. 0 .and. do_mbar) then
                  call pairs_calc_ti_%(type)s_cut2_mefv_vac(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)               
              else
                  call pairs_calc_ti_%(type)s_cut2_efv_vac(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)                   
              end if          
            end if
            ...
            if (cutoffs_equal) then
              if (ifmbar_lcl .ne. 0 .and. do_mbar) then            
                  call pairs_calc_ti_%(type)s_cut1_mefv(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)          
              else
                  call pairs_calc_ti_%(type)s_cut1_efv(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)               
              end if
            else
              if (ifmbar_lcl .ne. 0 .and. do_mbar) then
                  call pairs_calc_ti_%(type)s_cut2_mefv(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)               
              else
                  call pairs_calc_ti_%(type)s_cut2_efv(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)                   
              end if          
            end if
          end if
          ipairs_idx = ipairs_idx + ti_ee_eval_cnt(%(index)d) + ti_full_eval_cnt(%(index)d)
        end if
 
""" % {'index': i+2, 'type': pair[i]})
    ...
    f.write("""       
        if (ti_ee_eval_cnt(%(index)d) + ti_full_eval_cnt(%(index)d) .gt. 0) then         
          ...
          if(use_pme .eq. 0) then
            if (cutoffs_equal) then
              if (need_virials) then            
                  call pairs_calc_ti_%(type)s_cut1_fv_vac(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)          
              else
                  call pairs_calc_ti_%(type)s_cut1_f_vac(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)               
              end if
            else
              if (need_virials) then
                  call pairs_calc_ti_%(type)s_cut2_fv_vac(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)               
              else
                  call pairs_calc_ti_%(type)s_cut2_f_vac(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)                   
              end if          
            end if
            ...
            if (cutoffs_equal) then
              if (need_virials) then            
                  call pairs_calc_ti_%(type)s_cut1_fv(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)          
              else
                  call pairs_calc_ti_%(type)s_cut1_f(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)               
              end if
            else
              if (need_virials) then
                  call pairs_calc_ti_%(type)s_cut2_fv(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)               
              else
                  call pairs_calc_ti_%(type)s_cut2_f(img_crd, img_qterm, efs_tbl, &
                                     eed_cub, typ_ico, ipairs(ipairs_idx), &
                                     gbl_img_iac, gbl_cn1, gbl_cn2, x_tran)                   
              end if          
            end if
