...
!
!===============================================================================
module asm_CV_utilities_mod

        use asm_CV_mod, only : CV_bond, CV_angle, CV_dihedral, &
				                  CV_symdih, CV_asymdih, CV_pointplane
        use asm_utilities_mod, only : next_unit, write_error
        use asm_multiCV_mod
	...
	type CV
		...
		double precision, dimension(2) :: box
		logical :: periodic
	...
	double precision, dimension(:), allocatable :: CVs, M
...
contains
	...
	subroutine to_box(A)
		...
		double precision, dimension(nCV), intent(inout) :: A
	...
	subroutine get_M(Jacobian, M)
		...
		double precision, dimension(natom*3,nCV), intent(in) :: Jacobian
		double precision, dimension(msize), intent(out) :: M
	...
	!================================
	double precision function CV_distance(A, B, M)
	
		double precision, dimension(nCV), intent(in) :: A, B
	...
	!================================
	double precision function CV_distance2(A, B, M)
	...
	!================================
	double precision function dist_M(A, B, M)
	...
	!================================
	double precision function len_M(A, M)
	...
	!================================
	double precision function len2_M(A, M)
	...
	!================================
	double precision function dot_product_M(A, B, M)
	...
	function matmulp(M, A)
		...
		double precision, dimension(:), intent(in) :: M, A
		double precision, dimension(size(A)) :: matmulp
	...
	function pack_M(M)
		...
		double precision, dimension(nCV, nCV) :: M
		double precision, dimension(msize) :: pack_M
	...
	function unpack_M(M)
		...
		double precision, dimension(msize) :: M
		double precision, dimension(nCV, nCV) :: unpack_M
	...
	function map_periodic(A)
		...
		double precision, dimension(nCV), intent(in) :: A
		double precision, dimension(nCV) :: map_periodic
...
end module asm_CV_utilities_mod
