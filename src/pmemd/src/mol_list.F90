...
module mol_list_mod
  ...
  integer, allocatable                  :: atm_mol_map(:)
...
#else
  if (ti_mode .eq. 0) then !separate routine for ti because we need the
!ti_partner data passed to the gpu as well
    call gpu_molecule_list_setup(gbl_mol_cnt, gbl_mol_atms_listdata)
  end if
#endif
  if (natc .gt. 0) then
...
if(usemidpoint) then
  if(.not. allocated(atm_mol_map)) allocate(atm_mol_map(atm_cnt))
