module asm_multiCV_mod

    use asm_CV_mod
    use asm_utilities_mod, only : next_unit
    ...
    double precision, dimension(12) :: centers, centers_gradient
...
contains
    ...
    function get_center(indexes, x) result(center)
        ...
        double precision, dimension(:), intent(in) :: x
        double precision, dimension(3) :: center, reference, xyz
    ...
    function unpack_gradient(natoms, indexes, center_gradient) result(gradient)
        ...
        integer, dimension(:), intent(in) :: indexes
        double precision, dimension(:), intent(in) :: center_gradient

        integer :: i
        double precision, dimension(natoms*3) :: gradient
