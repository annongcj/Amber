...
! Formal arguments:
  ...
  character(1)          :: fform   ! format code: "U", "F" = unform., form.
...
! Local variables:
  ...
  character(11)         :: pos     ! position keyword
  ...
  if (fform .eq. 'U') then
    ...
    write(mdout, '(/,2x,a,i4)') 'amopen: bogus fform, unit', lun
    call mexit(6, 1)
  end if
...
! This doesn't look right; at the least it is unnecessary since mexit(6, 1)
! below will close unit 6; srb, 4/23/2020.
      close(unit = 6)
#ifndef DUMB
      write(0, '(/,2x,a,i4,a,a)') 'Unit ', lun, ' Error on OPEN: ', fname
#endif
             ...
             if (aline(j:j) .ne. ' ') then
                if (aline(j:j) .eq. '&' .or. aline(j:j) .eq. '$') then
                   if (80 - j + 1 .ge. ilen) then
                      if (upcase(aline(j + 1:j + ilen)) .eq. upcase(name)) goto 200
                   else
                      goto 10
                   end if
                else
      ...
      do 200 i = 1, lenstr
           if (string(i:i) .ge. '0'.and. string(i:i) .le. '9') then
                ...
                do 100 j = ifirst, lenstr
                     if (string(j:j) .lt. '0' .or. string(j:j) .gt. '9') then
                          last = j - 1
                          goto 300
                     end if
  100           continue
...
! If constrained groups read the weight for each group:
        ...
        if (konst) then
          ifld(1) = 3
          ifld(2) = 0
          call rfree(ifld, ihol, ivar, fvar, nf, mdout)
          wt = fvar(1)
...
! Find option ... Read the atom specifics:
          ...
          write(mdout, 200)
          do 64 iii = 1, 12
            call rfree(jfld, ihol, ivar, fvar, nf, mdout)
            jgraph(iii) = ihol(1)
            jsymbl(iii) = ihol(2)
            jtree(iii) = ihol(3)
            jresnm(iii) = ihol(4)
