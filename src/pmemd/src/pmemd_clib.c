...
#if !defined(NO_RLIMIT_STACK_CTRL) && !defined(WIN32)
...
void PROC_GET_BYTESIZE(void * start, void * end, int * bytes)
{
  *bytes = ((char *)end - (char *)start);
}

void PROC_GET_WALL_TIME(int * sec, int * usec)
{
...
void PROC_UNLIMIT_STACK(int * new_limit)
{
#if defined(NO_RLIMIT_STACK_CTRL) || defined(WIN32)
  *new_limit = -1;
#else
