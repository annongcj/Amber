...
subroutine pressure_scale_crds_midpoint(crd, mass, mol_mass_inv, mol_com)
...
! Local variables:
  ...
  call get_mol_com_midpoint(gbl_mol_cnt, crd, mass, mol_mass_inv, mol_com, old_pbc)

  last_com_midpoint(1:3,1:gbl_mol_cnt)=mol_com(1:3,1:gbl_mol_cnt)

  do mol_id = 1, gbl_mol_cnt

    ! Now get fracs for c.o.m. using old cell params

    frac1 = last_com_midpoint(1,mol_id) * last_recip(1, 1) + &
            last_com_midpoint(2,mol_id) * last_recip(2, 1) + &
            last_com_midpoint(3,mol_id) * last_recip(3, 1)

    frac2 = last_com_midpoint(1,mol_id) * last_recip(1, 2) + &
            last_com_midpoint(2,mol_id) * last_recip(2, 2) + &
            last_com_midpoint(3,mol_id) * last_recip(3, 2)

    frac3 = last_com_midpoint(1,mol_id) * last_recip(1, 3) + &
            last_com_midpoint(2,mol_id) * last_recip(2, 3) + &
            last_com_midpoint(3,mol_id) * last_recip(3, 3)
  ...
  do i = 1, proc_num_atms + proc_ghost_num_atms
    mol_id = atm_mol_map(proc_atm_to_full_list(i))
    wrap_offset(1) = proc_atm_wrap(1,i)*(pbc_box(1)-old_pbc(1))*(-1.0d0)
    wrap_offset(2) = proc_atm_wrap(2,i)*(pbc_box(2)-old_pbc(2))*(-1.0d0)
    wrap_offset(3) = proc_atm_wrap(3,i)*(pbc_box(3)-old_pbc(3))*(-1.0d0)
    crd(:, i) = crd(:, i) + wrap_offset(:) + mol_com(:,mol_id) - last_com_midpoint(:,mol_id)
  end do

  return

end subroutine pressure_scale_crds_midpoint
#endif
...
! Apply center of molecule based pressure scaling:
  ...
  if (usemidpoint) then
    call get_mol_com_midpoint(my_mol_cnt, crd, mass, mol_mass_inv, mol_com, pbc_box)
  else
    call get_mol_com(my_mol_cnt, crd, mass, mol_mass_inv, my_mol_lst, mol_com)
  endif
...
!
! Description: Scales unit cell uniformly by factor (isotropically);
...
! Recalculate volume and reciprocal matrix from new box dimensions to avoid long-term cumulative floating point roundoff error

  if (is_orthog .ne. 0) then
    uc_volume = ucell(1, 1) * ucell(2, 2) * ucell(3, 3)
    pbc_box(1) = ucell(1, 1)
    pbc_box(2) = ucell(2, 2)
    pbc_box(3) = ucell(3, 3)
    recip(1, 1) = 1.d0 / ucell(1, 1)
    recip(2, 2) = 1.d0 / ucell(2, 2)
    recip(3, 3) = 1.d0 / ucell(3, 3)
    reclng(1) = ucell(1, 1)
    reclng(2) = ucell(2, 2)
    reclng(3) = ucell(3, 3)
  else
    pbc_box(1) = ucell(1, 1)
    pbc_box(2) = sqrt(ucell(1, 2) * ucell(1, 2) + ucell(2, 2) * ucell(2, 2))
    pbc_box(3) = sqrt(ucell(1, 3) * ucell(1, 3) + ucell(2, 3) * ucell(2, 3) + ucell(3, 3) * ucell(3, 3))
    call cross(ucell(1, 2), ucell(1, 3), u23)
    call cross(ucell(1, 3), ucell(1, 1), u31)
    call cross(ucell(1, 1), ucell(1, 2), u12)
    call dot(ucell(1, 1), u23, uc_volume)
...
#ifdef MPI
  if(usemidpoint) box_len(:) = pbc_box(:)
#endif
...
subroutine wrap_molecules(crd)
  ...
  ! For TI:
  integer               :: offset_part
  integer               :: mol_id_part, mol_atm_cnt_part
  ...
  do mol_id = 1, gbl_mol_cnt
    ...
    if (ti_mode .ne. 0) then
      if (ti_mol_type(mol_id) .eq. ti_mol_part_sc_ex) then
        mol_id_part = ti_sc_partner(mol_id)
        if (mol_id_part .gt. 0) then
        mol_atm_cnt_part = gbl_mol_atms_listdata(mol_id_part)%cnt
        offset_part = gbl_mol_atms_listdata(mol_id_part)%offset
        if (mol_id_part .gt. mol_id) then
          do i = offset_part + 1, offset_part + mol_atm_cnt_part
            atm_id = gbl_mol_atms_lists(i)
            f1 = f1 + crd(1, atm_id)*recip(1, 1)+crd(2, atm_id)*recip(2, 1) + &
                      crd(3, atm_id)*recip(3, 1)
          ...
          mol_atm_cnt = mol_atm_cnt + mol_atm_cnt_part
        else
          cycle !already moved partner
        end if
      end if
    end if
    end if
    ...
    if (f1 .ne. g1 .or. f2 .ne. g2 .or. f3 .ne. g3) then
      ...
      if (ti_mode .ne. 0) then
        ...
        if (mol_id_part .gt. mol_id) then
          ...
          ! restore so we don't overflow below
          mol_atm_cnt = gbl_mol_atms_listdata(mol_id)%cnt
        end if 
        end if
        end if
...
!*******************************************************************************
!
! Subroutine:  proc_get_fract_crds (in range of 0.0 - +.999...)
!
! Description: <TBS>
!
!*******************************************************************************
subroutine proc_get_fract_crds(atm_cnt, fraction,int_x_dim, int_y_dim, int_z_dim)
  use parallel_dat_mod
  use processor_mod
...
! Formal arguments:
  ...
  integer               :: int_y_dim
  integer               :: int_z_dim
  double precision      :: fraction(3, proc_atm_alloc_size)
...
! Local variables:
  ...
  double precision      :: x_bkt_size, y_bkt_size, z_bkt_size

  if (is_orthog .ne. 0) then
!    x_bkt_size = pbc_box(1)/dble(sys_bkt_dimx)
!    y_bkt_size = pbc_box(2)/dble(sys_bkt_dimy)
!    z_bkt_size = pbc_box(3)/dble(sys_bkt_dimz)

    x_bkt_size = new_bkt_size(1)
    y_bkt_size = new_bkt_size(2)
    z_bkt_size = new_bkt_size(3)


  !we added 2, since two buckets are for ghosts
    recip_11 = 1.d0/(dble(int_x_dim + 2)*x_bkt_size) !  recip(1, 1)
    recip_22 = 1.d0/(dble(int_y_dim + 2)*y_bkt_size) !  recip(2, 2)
    recip_33 = 1.d0/(dble(int_z_dim + 2)*z_bkt_size) !  recip(3, 3)

!move the atm proc_atm_crd to its own block, considering the block is the whole system, and then again move one bucket to the right since we add 1 ghost bucket to the each side
    do i = 1, atm_cnt
      !proc_atm_crd is shifted to its block,so that recip_11 work on correct proc_atm_crd
      f1 = recip_11 * (proc_atm_crd(1, i) - proc_min_x_crd + x_bkt_size)
      f2 = recip_22 * (proc_atm_crd(2, i) - proc_min_y_crd + y_bkt_size)
      f3 = recip_33 * (proc_atm_crd(3, i) - proc_min_z_crd + z_bkt_size)
  ...
  ! and within the anticipated range here.
    ...
    do i = 1, atm_cnt
      ...
      f1 = proc_atm_crd(1, i) * recip(1, 1) + proc_atm_crd(2, i) * recip(2, 1) + &
           proc_atm_crd(3, i) * recip(3, 1)

      f2 = proc_atm_crd(1, i) * recip(1, 2) + proc_atm_crd(2, i) * recip(2, 2) + &
           proc_atm_crd(3, i) * recip(3, 2)

      f3 = proc_atm_crd(1, i) * recip(1, 3) + proc_atm_crd(2, i) * recip(2, 3) + &
           proc_atm_crd(3, i) * recip(3, 3)
      !later activate the 2nd set
      !fraction(1, i) = f1 - dnint(f1) + 0.5d0
      !fraction(2, i) = f2 - dnint(f2) + 0.5d0
      !fraction(3, i) = f3 - dnint(f3) + 0.5d0
      fraction(1, i) = f1
...
!remove it for performance, when the whole midpoint code is done
  do i = 1, atm_cnt
    !checking vlaues
     if (fraction(1, i) .lt. dble(1.d0/(dble(int_bkt_dimx)+2.d0))) print *, "underflow(x L)",fraction(1, i),&
                                                                  i,mytaskid,proc_atm_to_full_list(i)
     if (fraction(2, i) .lt. dble(1.d0/(dble(int_bkt_dimy)+2.d0))) print *, "underflow(y L)",fraction(2, i),&
                                                                            i,mytaskid,proc_atm_to_full_list(i)
     if (fraction(3, i) .lt. dble(1.d0/(dble(int_bkt_dimz)+2.d0))) print *, "underflow(z L)",fraction(3, i),&
                                                                        i,mytaskid,proc_atm_to_full_list(i)

     if (fraction(1, i) .ge. dble( (proc_crd_len(1)+new_bkt_size(1)) / (new_bkt_size(1)*dble(int_bkt_dimx+2)) )) &
                           print *, "overflow(x L)", fraction(1, i),i,mytaskid,proc_atm_to_full_list(i)
     if (fraction(2, i) .ge. dble( (proc_crd_len(2)+new_bkt_size(2)) / (new_bkt_size(2)*dble(int_bkt_dimy+2)) )) &
                           print *, "overflow(y L)", fraction(2, i),i,mytaskid,proc_atm_to_full_list(i)
     if (fraction(3, i) .ge. dble( (proc_crd_len(3)+new_bkt_size(3)) / (new_bkt_size(3)*dble(int_bkt_dimz+2)) )) &
                           print *, "overflow(z L)", fraction(3, i),i,mytaskid,proc_atm_to_full_list(i)

  end do

  return

end subroutine proc_get_fract_crds
!*******************************************************************************
!
! Subroutine:  proc_get_fract_crds_ghost (in range of 0.0 - +.999...)
!
! Description: <TBS>
!
!*******************************************************************************

subroutine proc_get_fract_crds_ghost(atm_cnt, ghost_atm_cnt, fraction,int_x_dim, int_y_dim, int_z_dim)

  use processor_mod, only: proc_min_x_crd, proc_min_y_crd, proc_min_z_crd, &
                           proc_max_x_crd, proc_max_y_crd, proc_atm_crd, &
                           proc_max_z_crd, proc_old_local_id, proc_crd_len, &
                           ghost_crd_len, new_bkt_size
  use parallel_dat_mod
...
!    z_bkt_size = pbc_box(3)/dble(sys_bkt_dimz)
  ...
  !we added 2, since two buckets are for ghosts
    recip_11 = 1.d0/(dble(int_x_dim + 2)*dble(x_bkt_size)) !  recip(1, 1)
    recip_22 = 1.d0/(dble(int_y_dim + 2)*dble(y_bkt_size)) !  recip(2, 2)
    recip_33 = 1.d0/(dble(int_z_dim + 2)*dble(z_bkt_size)) !  recip(3, 3)

!move the atm proc_atm_crd to its own block, considering the block is the whole system, and then again move one bucket to the right since we add 1 ghost bucket to the each side
    do i = atm_cnt+1,ghost_atm_cnt+atm_cnt
      f1 = proc_atm_crd(1,i)
      f2 = proc_atm_crd(2,i)
      f3 = proc_atm_crd(3,i)

      f1 = recip_11 * (f1 - (proc_min_x_crd) + (x_bkt_size))
      f2 = recip_22 * (f2 - (proc_min_y_crd) + (y_bkt_size))
      f3 = recip_33 * (f3 - (proc_min_z_crd) + (z_bkt_size))
  ...
  !do i = 1, atm_cnt
  do i = atm_cnt+1,ghost_atm_cnt+atm_cnt
     if (fraction(1, i) .lt. 0.d0) print *, "underflow(x g)",fraction(1,i)
     if (fraction(2, i) .lt. 0.d0) print *, "underflow(y g)",fraction(2,i)
     if (fraction(3, i) .lt. 0.d0) print *, "underflow(z g)",fraction(3,i)

     if (fraction(1, i) .ge. ghost_crd_len(1) * recip_11) &
                           write(0,"('overflow(x g)', 3F22.16)") fraction(1,i)&
      , ghost_crd_len(1) * recip_11, fraction(1,i)- ghost_crd_len(1) * recip_11
     if (fraction(2, i) .ge. ghost_crd_len(2) * recip_22) &
                           write(0,"('overflow(y g)', 3F22.16)") fraction(2, i)&
      , ghost_crd_len(2) * recip_22, fraction(2, i)- ghost_crd_len(2) * recip_22
     if (fraction(3, i) .ge. ghost_crd_len(3) * recip_33) &
                           write(0,"('overflow(z g)', 3F22.16)") fraction(3, i)&
      , ghost_crd_len(3) * recip_33, fraction(3, i)- ghost_crd_len(3) * recip_33

  end do

  return

end subroutine proc_get_fract_crds_ghost
