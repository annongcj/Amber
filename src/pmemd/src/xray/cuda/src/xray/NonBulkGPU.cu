    ...
    for (unsigned hkl_offset = blockIdx.x * WARP_SIZE; hkl_offset < n_hkl; hkl_offset += gridDim.x * WARP_SIZE) {
      ...
      for (unsigned global_atom_offset = 0; global_atom_offset < n_atom; global_atom_offset += BLOCK_SIZE) {
        ...
        for (unsigned hkl_cached_idx = warp_idx;
          ...
          f_real += __SHFL_DOWN(warp_mask, f_real, 16);
          f_real += __SHFL_DOWN(warp_mask, f_real, 8);
          f_real += __SHFL_DOWN(warp_mask, f_real, 4);
          f_real += __SHFL_DOWN(warp_mask, f_real, 2);
          f_real += __SHFL_DOWN(warp_mask, f_real, 1);

          f_imag += __SHFL_DOWN(warp_mask, f_imag, 16);
          f_imag += __SHFL_DOWN(warp_mask, f_imag, 8);
          f_imag += __SHFL_DOWN(warp_mask, f_imag, 4);
          f_imag += __SHFL_DOWN(warp_mask, f_imag, 2);
          f_imag += __SHFL_DOWN(warp_mask, f_imag, 1);
