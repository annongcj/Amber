...
contains
   ...
   subroutine grid_atomic_density( &
      ...
      use constants_module, only: M_PI => PI, M_FOURPI => FOURPI
      ...
      write(6,*)'num scatter types=',num_scatter_types
      write(6,*)'num_atoms = ',num_atoms
      write(6,'(A,3F8.3)') 'FFT grid spacing = ',angstrom_to_grid**(-1)

      ! ----------------------------------------------------------------------
      density_calc = .not. ( present(d_tempFactor) &
                     .or. present(d_occupancy) .or. present(d_XYZ) )
      write(6,*) 'density_calc: ', density_calc
      ...
      do iatom = 1,num_atoms
         ...
         !write(6,*)'tempFactor=',tempFactor(iatom)
         !write(6,*) 'sharpen=', fft_bfactor_sharpen
         !write(6,*)'acoeffs=',acoeffs
         !write(6,*)'nbcoeffs=',nbcoeffs
         ...
         if (num_points == 0) then
            write(6,'(A)') 'SPHERE SIZE IS ZERO!'
            write(6,*) 'grid_size = ',grid_size(:)
            write(6,*) 'rcut = ',rcut
            write(6,*) 'angstrom_to_grid = ',angstrom_to_grid(:)
            call mexit(6,1)
         end if
   ...
   contains
      subroutine get_real_space_coeffs(iatom,acoeffs,nbcoeffs,rcut)
         ...
         do i = 1,num_scatter_coeff
            ...
            if (b<0.1) then
               write(6,'(2i5,4f10.3)') i,iatom,scatter_coeffs(2,i,scatter_type(iatom)), &
                  tempFactor(iatom),fft_bfactor_sharpen,b
