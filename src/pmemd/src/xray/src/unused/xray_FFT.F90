...
!
!  FFT_grid_to_hkl--  This reduces the FFT grid to the observerved HKL list.
...
!
   ...
   use constants_module, only: M_TWOPI => TWOPI
...
contains
   ...
   subroutine FFT_Fcalc( &
      ...
      use xray_utils_module, only: allocate_lun, write_map_ccp4
      implicit none
      ...
      write(6,*) 'time for grid_atomic_density: ', time1-time0
      time0 = time1
      write(6,'(6f10.3)') density_map(1,1,:)
      stop

#if 0
      call amopen(allocate_lun(unit),'xray_atom.ccp4','U','F','R')
      call write_map_ccp4(unit, &
      ...
      cmap(:,:,:) = density_map(:,:,:)
      call fft3c( fft3_desc, cmap, error )
      call wallclock(time1)
      write(6,*) 'time for fft3c: ', time1-time0
      fft_normalization = 1.d0

      !   dac: in above, I think the cmap is overwritten with
      !        what is sometimes called recip_map below (?)

#if 1
      call FFT_grid_to_hkl(density_map,num_hkl,hkl_index,Fcalc,hkl_selected)
      stop
#else
      ! dac: looks like most of this repeats what is done in
      !      FFT_grid_to_hkl()  (?), except: the latter handles symops,
      !      and this routine has a b-sharpening component....
      ...
      do i = 1,num_hkl
         ...
         if (hkl_index(1,i)<0) then
            ih = -hkl_index(1,i)
            ik = -hkl_index(2,i); if (ik<0) ik = ik + recip_size(2)
            il = -hkl_index(3,i); if (il<0) il = il + recip_size(3)

            ! Fcalc(i) = fft_normalization &
            !     * cmplx(recip_grid(1,ih,ik,il), recip_grid(2,ih,ik,il)) &
            !     * exp( fft_bfactor_sharpen * mSS4(i))

            Fcalc(i) = fft_normalization * cmap(ih,ik,il)
         else
            ih = hkl_index(1,i)
            ik = hkl_index(2,i); if (ik<0) ik = ik + recip_size(2)
            il = hkl_index(3,i); if (il<0) il = il + recip_size(3)
         ...
         write(6,'(3i5,2f15.5)') hkl_index(:,i), Fcalc(i)
      end do
      stop
#endif
   ...
   subroutine FFT_dXYZBQ_dF(&
      ...
      do isym = 1,num_symmops
         do ihkl = 1,num_hkl
            ...
            if (hkl_index(1,ihkl) <= 0) then
               ih = -hkl_index(1,ihkl)
               ik = -hkl_index(2,ihkl); if (ik<0) ik = ik + recip_size(2)
               il = -hkl_index(3,ihkl); if (il<0) il = il + recip_size(3)
   ...
   subroutine FFT_hkl_to_grid(recip_grid, &
      ...
      do isym = 1,num_symmops
         ...
         do ihkl = 1,num_hkl
            ...
            f = cmplx(cos(phase),sin(phase),real_kind)
   ...
   subroutine FFT_grid_to_hkl(recip_grid, &
      ...
      do ihkl = 1,num_hkl
         ...
         refl(ihkl) = fft_normalization * cmplx(r(1),r(2),real_kind)
         write(20,'(3i5,2f15.5)') symm_hkl, refl(ihkl)
      end do !ihkl
      ...
      do isym = 2,num_symmops
         ...
         do ihkl = 1,num_hkl
            ...
            r = recip_grid(:,symm_hkl(1),symm_hkl(2),symm_hkl(3))
            if (Friedel) r(2) = -r(2) !conjg(r)
            refl(ihkl) = refl(ihkl) + f * fft_normalization *  &
                         cmplx(r(1),r(2),real_kind)
