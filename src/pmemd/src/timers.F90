...
! NOTE: fcve is frc-crds-vel-ene
  ...
  integer, parameter    :: ihkl_time                    = 10
  integer, parameter    :: dhkl_time                    = 11
  integer, parameter    :: max_generic_timer            = 11
...
#if defined(_OPENMP_)
!additional timer for packing data
  integer, parameter    :: get_nb_pack_timer      = max_generic_timer + 14
  integer, parameter    :: nb_pair_pack_timer     = max_generic_timer + 15
  integer, parameter    :: max_pme_timer           = max_generic_timer + 15
#else
...
#ifndef MPI
#ifndef NOXRAY
  use xray_globals_module, only: ihkl_duration, dhkl_duration, xray_active
#endif
#endif
  implicit none
...
#endif
#ifdef _OPENMP_
  time_stats(nonsetup_time) = run_end_walltime - run_setup_end_walltime
#else
...
#ifndef NOXRAY
  if (xray_active) then
    time_stats(ihkl_time) = ihkl_duration
    time_stats(dhkl_time) = dhkl_duration
  else
#endif
#endif
    time_stats(ihkl_time) = 0.d0
    time_stats(dhkl_time) = 0.d0
#ifndef MPI
#ifndef NOXRAY
  end if
#endif
...
#ifdef _OPENMP_
 if(time_stats(other_time) .lt. 0.d0) time_stats(other_time) = 0.d0
#endif
  ...
  if(time_stats(nonsetup_time) .ne. 0.0) then
...
#ifndef NOXRAY
  if (xray_active) then
    write(mdout, 20) 'Calc ihkl  ', ihkl_duration, percent(ihkl_time)
    write(mdout, 20) 'Calc dhkl  ', dhkl_duration, percent(dhkl_time)
  end if
#endif
#endif
  write(mdout, 20) 'Bond       ', &
...
subroutine zero_time()
  ...
  current_time = dble(wall_sec) + dble(wall_usec) / 1000000.0d0
  generic_time1 = current_time
#else
...
! Local variables
  ...
  if(step_interval .lt. 1) step_interval = 1 ! to avoid divide by zero, investigate later why step_interval is zero sometimes
