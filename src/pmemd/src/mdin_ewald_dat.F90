...
!
! skinnb = 2.0 - The width of the non-bonded skin (pair list margin).  When
!                particles have travelled half this distance, the pair list is
!                no longer valid.  A permittivity (skin_permit) is included to  
!                allow particles to travel beyond the safe limit, running a
!                slight risk of missing a peripheral non-bonded interaction
!                in order to build the pair list less frequently and save time.  
!
! skin_permit = 0.5 - The permissible factor of the non-bonded pair list margin
!                     over which particles may travel before triggering a pair
!                     list rebuild.
  ...
  integer, parameter    :: mdin_ewald_dbl_cnt = 6

  save  :: /mdin_ewald_int /

  double precision              dsum_tol, ew_coeff, skinnb, eedtbdns, &
                                fft_grids_per_ang, skin_permit
  ...
  double precision, parameter   :: skin_perm_lo = 0.5d0
  double precision, parameter   :: skin_perm_hi = 1.0d0
  ...
  double precision              :: target_a, target_b, target_c
  integer                       :: target_n
...
! Namelist entries that are returned with disambiguated names for clarity:
  ...
  namelist /ewald/      nfft1, nfft2, nfft3, order, verbose, ew_type, &
                        ...
                        atm_redist_freq, skin_permit, &
                        target_a, target_b, target_c, target_n
...
! Optimal values for skinnb selected below (or best guess anyway).
  ...
  ! The van der Waals long-range correction is incompatible with the
  ! force-switch on van der Waals interactions, so turn it off here
...
! Check on bogus data and unsupported options:
  ...
  call float_legal_range('skin_permit', skin_permit, skin_perm_lo, skin_perm_hi, inerr)
  ...
  if (order .ne. 4 .and. order .ne. 6) then
    write(mdout, '(a)') 'CUDA (GPU): Implementation does not support interpolation orders '
    write(mdout, '(a)') '            other than 4 or 6.'
    inerr = 1
  end if
#endif

! Field any errors and bug out.
