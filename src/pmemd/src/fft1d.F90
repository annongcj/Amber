...
#include <fftw3.f>
  ...
  interface
    subroutine sfftw_execute_dft(plan, in, out)
      integer*8         :: plan(*)
      pme_float         :: in(*)
      pme_float         :: out(*)
    end subroutine sfftw_execute_dft
  end interface
#else /*pmemd_SPDP*/
  interface
...
subroutine fft1d_create(fft_hdl, fft_size, num_ints, num_reals)
...
#ifdef pmemd_SPDP
  call sfftw_plan_dft_1d(fft_hdl%backward_plan, fft_size, &
                         tmp_buf, tmp_buf, FFTW_BACKWARD, FFTW_MEASURE)

  call sfftw_plan_dft_1d(fft_hdl%forward_plan, fft_size, &
...
subroutine fft1d_destroy(fft_hdl, num_ints, num_reals)
...
#ifdef FFTW_FFT
#ifdef pmemd_SPDP
  call sfftw_destroy_plan(fft_hdl%backward_plan)
  call sfftw_destroy_plan(fft_hdl%forward_plan)
#else /*pmemd_SPDP*/
...
#ifdef FFTW_FFT
#ifdef pmemd_SPDP
  call sfftw_execute_dft(fft_hdl%forward_plan, fft_array, fft_array)
#else /*pmemd_SPDP*/
...
#ifdef FFTW_FFT
#ifdef pmemd_SPDP
  call sfftw_execute_dft(fft_hdl%backward_plan, fft_array, fft_array)
#else /*pmemd_SPDP*/
...
#ifdef CUDA
      ' must be a multiple of 4 with prime factors of only 2,3,5,7 for fft efficiency!' 
#else
#ifdef FFTW_FFT
...
!               a product, 0 if not.  For fftw and cuda, we also permit a prime
!               factor of 7.
!*******************************************************************************
...
! Insure multiple of 4 when using CUDA
#ifdef CUDA
    if (4 * (n / 4) .ne. n) then
      result = 0
      return
...
#ifdef pmemd_SPDP
  real      :: taur, taui, tr2, cr2, ti2, ci2, cr3, ci3, &
                           dr2, dr3, di2, di3
#else
...
#ifdef pmemd_SPDP
  real      :: ti2, ti3, ti4, ti5, ti11, ti12
  real      :: tr2, tr3, tr4, tr5, tr11, tr12
  real      :: ci2, ci3, ci4, ci5
  real      :: cr2, cr3, cr4, cr5
  real      :: di2, di3, di4, di5
  real      :: dr2, dr3, dr4, dr5
#else
...
! Local variables:
  ...
  real      :: di2, di3, di4, di5
  real      :: dr2, dr3, dr4, dr5
  real      :: ti2, ti3, ti4, ti5
  real      :: tr2, tr3, tr4, tr5
