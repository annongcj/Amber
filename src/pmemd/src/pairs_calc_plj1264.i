...
subroutine pairs_calc_efv_2cut(img_frc, img_crd, img_qterm, ef_tbl, eed_cub, &
                               ico, ipairs_sublst, img_iac, cn1, cn2, & ! New2022
                               cn7, cn8, C4Pairwise, x_tran) ! New2022
#else
subroutine pairs_calc_efv(img_frc, img_crd, img_qterm, ef_tbl, eed_cub, &
                          ico, ipairs_sublst, img_iac, cn1, cn2, cn7, cn8, C4Pairwise, x_tran) ! New2022
...
! Formal arguments:
  ...
  integer                       :: cn7(*)
  double precision, intent(in)  :: cn8(*)
      ...
      if (ic .gt. 0) then
        ...
        do im_c4 = 1, C4Pairwise                                                        
          ...
          .and. MAX(img_i,img_j) == (MAX(cn7(im_c4*3-2), cn7(im_c4*3-1))+3)/3) then            
            f4 = f4 + cn8(im_c4)*mr4        
            write (6, '(4i8)') 1, C4Pairwise, img_i, img_j ! New2022                                          
          end if                                                                       
        end do    
        !write (6, '(5i8)') iaci, img_i, img_j, nxt_img_j, img_iac(img_j) ! New2022
        !if ((img_i .eq. cn7(1)) .and. (img_j .eq. cn7(2))) then
        !  f4 = mr4 * cn8(1) ! New2022
        !  write (6, '(a)') 'calculation go brrrrr' ! New2022Debug
        !else 
        !  f4 = 0.0
        !end if
#ifdef NEED_ENE       
