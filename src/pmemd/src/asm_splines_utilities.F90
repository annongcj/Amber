module asm_splines_utilities_mod
...
contains
	...
	double precision function spline_value( x, coef )
	
		double precision, intent(in) :: x
		double precision, dimension(:,:),intent(in) :: coef
		
		integer :: idx
		double precision :: x_max, y_max, k
	...
	double precision function spline_min(coef)
	...
	double precision function spline_max(coef)
	...
	double precision function spline_der( x, coef ) !Derivative dy(x)/dx
	...
	double precision function spline_integrate( x_min, x_max, coef )
	...
	subroutine CubicSplines( x, y, coef )
		...
		double precision, dimension(size(x)-1,5), intent(out) :: coef

		integer :: i, n
	
		double precision, dimension(size(x)-1) :: h
		double precision, dimension(size(x)-2) :: d, b
		double precision, dimension(size(x)) :: s
	...
	subroutine CubicSplinesND( x, y, coef )
		...
		double precision, dimension(size(x)-1,5,size(y, dim=1)), intent(out) :: coef
	...
	function spline_value_ND(x, coef)
		...
		double precision, intent(in) :: x
		double precision, dimension(:,:,:), intent(in) :: coef
		double precision, dimension(size(coef, dim=3)) :: spline_value_ND
	...
	double precision function spline_grad_mod_ND(x, coef)
	...
	function spline_der_ND(x, coef)
		...
		double precision, intent(in) :: x
		double precision, dimension(:,:,:), intent(in) :: coef
		double precision, dimension(size(coef, dim=3)) :: spline_der_ND
	...
	subroutine CubicSplinesFit( a, b, coef )
		...
		double precision, dimension(:), intent(in) :: a, b	  !Vectors with coordinates of data points
		double precision, dimension(:,:), intent(out) :: coef !coef(:,1) - x_i values, coef(:,2:5) - spline coefficients
		
		integer :: n, np	!number of data points
		integer :: i, j, idx
		double precision :: h		! x_i+1 - x_i
		double precision, dimension(size(coef, dim=1)+1) :: x, y	!Coordinates of spline reference points
		integer, dimension(size(a)) :: aidx	!indexes of splines to which each data point belongs
		double precision, dimension(size(coef, dim=1)+1) :: Mval !Final M values
		double precision, dimension(size(coef, dim=1)+1) :: cpb
		double precision, dimension(size(coef, dim=1)+1,size(coef, dim=1)+1) :: M  !m = My
		double precision, dimension(size(a),size(coef, dim=1)+1) :: C !C_ji = d/dy_i S(a_j)
		double precision, dimension(size(coef, dim=1)+1,size(a)) :: CT
		double precision, dimension(size(coef, dim=1)+1,size(coef, dim=1)+1) :: Cp !Coefficient matrix for the final equation system
	...
	subroutine solve(n, A, b, x)
		...
		double precision, dimension(n,n) :: Mtmp
	...
	function SplineMatrix( n, h ) 
		...
		double precision, dimension(n,n) :: SplineMatrix
		double precision, dimension(n-2,n-2) :: X
		integer, dimension(n-2,n) :: Y
		
		integer :: i, j
		double precision, parameter  :: lambda = 1.31695789692482_8
	...
	!WARNING! d and b values are not conserved!
		...
		integer, intent(in) :: n
		double precision, dimension(:), intent(inout) :: d, subd, supd, b
		double precision, dimension(n) :: Thomas
...
end module asm_splines_utilities_mod
