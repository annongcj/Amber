...
! NOTE: no mpi functions may be called after a call to mpi_finalize.
  ...
  if (i .eq. 0) then
    ...
    write (0,"(A)") "STOP PMEMD Terminated Abnormally!"
    call exit(1)
  end if
...
!*******************************************************************************
!
! Subroutine:  setup_dealloc_error
!
! Description: Error subroutine to call if global allocation failed
!              
!*******************************************************************************
...
subroutine setup_dealloc_error
  ...
  write(mdout, '(1x,2a)') 'FATAL global dynamic memory deallocation error!'

  call mexit(6, 1)

end subroutine setup_dealloc_error
...
!
! Description: Strips comments from the passed string. Take a single character
!              as the comment indicator (defaults to !)
!
!*******************************************************************************

subroutine strip_comments(input_string, comment_char)

  implicit none

! Passed arguments

  character(*), intent(in out)       :: input_string
  character(*), intent(in), optional :: comment_char

! Local variables

  integer :: i       ! counter
  integer :: cmt_loc ! Location of the comment character
  character :: cmt   ! comment character

  ! Set up the default comment character
  ...
  ! Set the comment location at the end, and loop through the string. If the
  ! comment character is found, it will be at a location smaller than the length
  ! of the trimmed string, so we update cmt_loc so long as cmt_loc is > i (this
  ! avoids resetting the cmt_loc to a second comment token if it's found)

  cmt_loc = len_trim(input_string) + 1
  do i = 1, len_trim(input_string)
    if (input_string(i:i) .eq. cmt .and. cmt_loc .ge. i) then
      cmt_loc = i
    end if
  end do

  input_string = input_string(1:cmt_loc-1)
...
! Subroutine: upper
!
! Description: Make everything uppercase
!
!*******************************************************************************
...
! Subroutine: strip
!
! Description: Strips leading whitespace (and trailing, but the length of the
! string in Fortran can't really be changed, so it's effectively just leading
! whitespace)
!
!*******************************************************************************
...
! Local variables
  ...
  if( upcase(atom_name(1:1)) .eq. 'A' ) then
    ...
    !Note Ep1 and Epw are extra points - they should have a mass of zero
    !     Assume Extra point atomic numbers are 0.
    if(atom_mass <= 0.01d0) then
      atomic_number = 0 !Extra Point
    else if (atom_mass > 145.0d0 .and. atom_mass <= 160.0d0) then
      atomic_number = 63 !Europium
    else if (atom_mass > 160.0d0 .and. atom_mass <= 175.0d0) then
      atomic_number = 68 !Erbium
    else if (atom_mass > 240.0d0 .and. atom_mass <= 280.0d0) then
      atomic_number = 99 !Einsteinium
    else
      localErrorFlag=.true.
    end if
