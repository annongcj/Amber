...
!  save  :: / ene_frc_splines_dbl /
  ...
  double precision, allocatable, save   :: efs_tbl(:)   ! energies & forces
  double precision, allocatable, save   :: fs_tbl(:)    ! only forces
...
subroutine init_ene_frc_splines_dat(num_ints, num_reals)
...
! write(6,*)'DBG: efs_tbl_siz = ', efs_tbl_siz
  ...
  call alloc_ene_frc_splines_mem(num_ints, num_reals)
...
subroutine alloc_ene_frc_splines_mem(num_ints, num_reals)
...
! Local variables:
  ...
  ! coefficients per tau value.  The fs_tbl is only the forces.

  allocate(efs_tbl(2 * 4 * efs_tbl_siz), &
           fs_tbl(1 * 4 * efs_tbl_siz), &
           stat = alloc_failed)

  if (alloc_failed .ne. 0) call setup_alloc_error

  num_reals = num_reals + size(efs_tbl) + size(fs_tbl)
...
subroutine fill_ef_spline_table
  ...
  ! coefficients to avoid calcs in the pair calc routines.
  ...
  ! We now also provide a forces-only table, for when energies are not needed.

  do i = 1, efs_tbl_siz
    j = 4 * (i - 1)
    fs_tbl(1 + j) = frc_spline_tbl(1 + j)
    fs_tbl(2 + j) = frc_spline_tbl(2 + j)
    fs_tbl(3 + j) = frc_spline_tbl(3 + j) * half
    fs_tbl(4 + j) = frc_spline_tbl(4 + j) * sixth
  end do
...
subroutine fill_ene_spline_table
  ...
  ! simulation to blow up, as we are basically dealing with a function with r
...
! Local variables:
  ...
  if (master) then
    if (frc_err_lim_exceeded_at .ge. 0.d0) then
...
1003 format('| Relative Error Limit not exceeded for r .gt. 0.d0') ! unlikely!
  ...
  if (ene_err_lim_exceeded_at .ge. 0.d0 .and. &
      frc_err_lim_exceeded_at .ge. 0.d0) then
    lowest_efs_delr2 = &
      max(ene_err_lim_exceeded_at, frc_err_lim_exceeded_at) + del
  else
    lowest_efs_delr2 = 0.d0
  end if
  
  if(use_pme .eq. 0) then
    lowest_efs_delr2 = 999999.d0 
  end if
