      ...
      if (ti_region .gt. 0) then
        ...
        if (gbl_atm_owner_map(i) .eq. mytaskid) then
          ...
          if (ti_mode .eq. 1 .or. ti_lscale) then   
            call ti_update_ene(epw, si_dihedral_imp_ene, ti_region, 2)
            epw = epw * ti_item_weights(2,ti_region)
          else
            ti_ene(ti_region,si_dihedral_imp_ene) = &
              ti_ene(ti_region,si_dihedral_imp_ene) + epw
            epw = 0.d0
          end if
#ifdef MPI
        end if
#endif
        if (ti_mode .eq. 1 .or. ti_lscale) &
          function_val = function_val * ti_item_weights(2,ti_region)
      end if
    end if
