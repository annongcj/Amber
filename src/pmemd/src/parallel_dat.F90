...
! Global variables necessary for MPI. These are NOT broadcast. All are VGB
  ...
  integer, save         :: mytaskid_recip ! rank reciprocal task in pmemd replica
  integer, save         :: numtasks_recip ! size of reciprocal tasks in the pmemd communicator
  integer, save         :: err_code_mpi  ! error code for MPI subroutine calls
  integer, save         :: ierr_DG       !DG to test pme_force mpi calls
  ...
  integer, save         :: remd_rank     ! rank in remd_group
  integer, save         :: remd_size     ! size of remd_comm

  integer, save         :: master_size
  ! moved from multipmemd.F90, by FENG PAN

  integer, save         :: repnum        ! replica number (master_rank + 1)
                                         ! visible to all of pmemd_comm
...
! pmemd_master_comm. All are VGB
  ...
  integer, save         :: pmemd_comm_number  ! Numeric ID for replica comm
  integer, save         :: pmemd_group        ! intra-replica group
  integer, save         :: pmemd_master_comm  ! comm linking replica masters

! Replica exchange comm variables

  integer, save         :: remd_comm          ! comm for given REMD group
  integer, save         :: group_master_comm  ! comm b/w REMD group masters
  integer, save         :: group_master_rank  ! rank in group_master_comm
  integer, save         :: num_rem_grp        ! number of REMD groups
  logical, save         :: remd_master        ! master of a REMD group
...
#ifdef _OPENMP_
  integer, allocatable, save                         :: gbl_atm_tid_map(:)
#endif 
...
! double assign a value...
  ...
  ! For getting MPI error messages string 
  integer ::  mpi_error_msg_len , mpi_err_tmp
  character, save :: mpi_err_msg_str(MPI_MAX_ERROR_STRING)
...
#ifdef _OPENMP_
allocate(gbl_atm_tid_map(atm_cnt), &
         stat = alloc_failed)

  if (alloc_failed .ne. 0) call setup_alloc_error
  num_ints = num_ints + size(gbl_atm_tid_map)
