...
public:
  ...
  int ischeme;          // middle-scheme setting
  int ithermostat;      // middle-scheme thermostat setting
  ...
  GpuBuffer<unsigned int>*   pbAtomIonMask;       // Atom is ion (>=1) or not (0)
  ...
  GpuBuffer<PMEAccumulator*>* pbPeerAccumulatorList;
                                                  // List of accumulator buffer addresses
  cudaIpcMemHandle_t* pPeerAccumulatorMemHandle;  // Memory handles for each peer accumulator
  GpuBuffer<unsigned int>*   pbPersistentStages;  // Values for communications between host and
                                                  // device in persistent allreduce kernels
...
#ifdef VKFFT
  VkFFTApplication* appR2C;                       // Configured VkFFT for FFT and IFFT
  uint64_t inputBufferSize;                       // Internal variable referenced by appR2C
  uint64_t outputBufferSize;                      // Internal variable referenced by appR2C
#endif
#ifdef use_DPFP
  ...
  size_t sortTempBytes;           // Temporary bytes needed for CUB sort
  GpuBuffer<char>* pbSortTemp;    // CUB GPU temporary data buffer
  ...
  GpuBuffer<unsigned int>* pbFinishedBlocksCounters; // Counters of finished blocks in
                                                     // cudaDeviceSynchronize-less kernels
  GpuBuffer<unsigned int>* pbKernelCompletionFlags;  // Completion flags for
                                                     // cudaDeviceSynchronize-less kernels
  ...
  GpuBuffer<int4>*   pbImageSlowShakeID2;            // Remapped XH4 (Slow) SHAKE constraint
                                                     //   hydrogens
  GpuBuffer<int>*    pbImageSlowTIShakeID1;          // Remapped Central atom of XH4 (Slow)
                                                     //   Shake constraint
  GpuBuffer<int4>*   pbImageSlowTIShakeID2;          // Remapped XH4 (Slow) SHAKE constraint
                                                     //   hydrogens
  GpuBuffer<int4>*   pbImageSlowTIShakeID3;          // Remapped XH4 (Slow) SHAKE constraint
                                                     //   hydrogens
  ...
  unsigned int randomCounterPHMD;                 // Counter for triggering RNG for pHMD
  bool use_linked;                                // Should we use linked water?
  int iphmd;                                      // Version flag for pHMD
  double sumq;                                    // sum of charges for iphmd=3
  double sumq2;                                   // Sum of squared charges for iphmd = 3

  GpuBuffer<PMEFloat>*       para;                 // Parameters for pHMD
  GpuBuffer<PMEFloat>*       parb;                 // Parameters for pHMD
  GpuBuffer<PMEFloat>*       park;                 // Parameters for pHMD
  GpuBuffer<PMEFloat>*       barr;                 // Parameters for pHMD
  GpuBuffer<PMEFloat>*       parmod;               // Parameters for pHMD (6xn in CPU code)
  GpuBuffer<PMEFloat2>*      sp_par;               // Parameters for pHMD

  GpuBuffer<PMEFloat2>*    qstate1;              // Protonated charges for tautomeric states
  GpuBuffer<PMEFloat2>*    qstate2;              // Deprotonated charges for tautomeric states
  GpuBuffer<PMEFloat2>*    ImageQstate1;         // Protonated charges for tautomeric states, image
  GpuBuffer<PMEFloat2>*    ImageQstate2;         // Deprotonated charges for tautomeric states, image
  GpuBuffer<PMEFloat2>*    vstate1;              // Flags whether an atom is present in protonated state
  GpuBuffer<PMEFloat2>*    vstate2;              //Flags whether an atom is preasent in deprotonated state
  GpuBuffer<PMEFloat2>*    ImageVstate1;         // Flags whether an atom is present in protonated state, image
  GpuBuffer<PMEFloat2>*    ImageVstate2;         //Flags whether an atom is preasent in deprotonated state, image
  GpuBuffer<PMEFloat2>*    qstate1_md;           //qstate1 but for MD charges
  GpuBuffer<PMEFloat2>*    qstate2_md;           //qstate2 but for MD charges

  GpuBuffer<PMEFloat>*     charge_phmd;          // Charges in titration calculations
  GpuBuffer<PMEFloat>*     ImageCharge_phmd;     // Charges in titration calculations, image

  GpuBuffer<int>*          sp_grp;               // Type of titrating residues
  GpuBuffer<int>*          grplist;              // Id's whether an atom is titrating
  GpuBuffer<int>*          ImageGrplist;         // Id's whether an atom is titrating, image
  GpuBuffer<int>*          ImageIndexPHMD;       //Stores image ids
  GpuBuffer<PMEFloat>*       ph_theta;             // Titration variables
  GpuBuffer<PMEFloat>*       dph_theta;            // Titration forces
  GpuBuffer<PMEAccumulator>* dph_accumulator; // Titration force accumulator
  GpuBuffer<PMEAccumulator>* dph_plus_accumulator; // Titration force accumulator
  GpuBuffer<unsigned long long int>*       phmd_ene;             //pHMD energy accumulator
  GpuBuffer<PMEFloat>*       vph_theta;            // Velocities of titration variables
  GpuBuffer<PMEFloat>*       dudls;
  GpuBuffer<PMEFloat>*       dphold;               // Hold variables for forces
  GpuBuffer<PMEFloat>*       vphold;               // Hold variables for velocities
  GpuBuffer<double>*       pbRandomPHMD;         // Random number buffer for pHMD
  GpuBuffer<int2>*         resbounds;            // Hold first and last atoms in each titrating residue
  GpuBuffer<bool>*         baseres;              // Is the residue a base residue?
  GpuBuffer<int>*          excludeatoms;         // Atoms excluded from GB radii calculations
  GpuBuffer<int>*          titratoms;            // Titrating atom indices
  GpuBuffer<int>*          linkgrplist;          // Linked residue list
  ...
  GpuBuffer<int>*        pbNMRr6avDistanceWeights;  // NMR r6av distance weights
  GpuBuffer<PMEDouble>*  pbNMRr6avDistanceXYZ;      // NMR r6av X,Y,Z distance components
  ...
  // MCWAT Buffers
  GpuBuffer<PMEFloat>* pbStericRadius;       // Radius for each atom to generate steric grid
  GpuBuffer<int>* pbVoxelOffset;       // Radius for each atom to generate steric grid
  GpuBuffer<unsigned int>* pbStericGrid;     // Steric grid
  GpuBuffer<int>* pbStericEmptyVoxels;       // Empty voxels in steric grid
  GpuBuffer<int>* pbStericThreadCount;       // Counts for each thread in steric grid
  GpuBuffer<int>* pbCoarseGrid;              // Coarse Grid
  GpuBuffer<int>* pbRandomWater;             // Random water atom ids for coarse grid
  GpuBuffer<PMEFloat>* pbCoarseTrans;        // Water translation for coarse grid
  GpuBuffer<PMEFloat>* pbCoarseGridEne;      // Coarse grid energy for a water move


    // SGLD  Buffers
    GpuBuffer<int>*             pbWsg;                 //  SGLD weighters
    GpuBuffer<double>*          pbX0sg;                //   1st average of crd
    GpuBuffer<double>*          pbX1sg;                //   1st average of crd
    GpuBuffer<double>*          pbX2sg;                //   2nd average of crd
    GpuBuffer<double>*          pbRsg;                //  SGLD random forces
    GpuBuffer<double>*          pbFPsg;                //  SGLD force times momentum
    GpuBuffer<double>*          pbPPsg;                //  SGLD momentum times momentum
    GpuBuffer<int>*             pbImageWsg;                 //  SGLD weighters
    GpuBuffer<double>*          pbImageX0sg;           // Image SGLD 1st average of crd
    GpuBuffer<double>*          pbImageX1sg;           // Image SGLD 1st average of crd
    GpuBuffer<double>*          pbImageX2sg;           // Image SGLD 2nd average of crd
    GpuBuffer<double>*          pbImageRsg;           // Image SGLD random forces
    GpuBuffer<double>*          pbImageFPsg;           // Image SGLD force times momentum
    GpuBuffer<double>*          pbImagePPsg;           // Image SGLD momentum times momentum
    GpuBuffer<SGLDAverage>*     pbSGLDAverageBuffer;      // SGLD average accumulation buffer
  ...
  GpuBuffer<double>* pbExtraPoint11;             // Type 1 single extra point local coordinates
  ...
  GpuBuffer<double>* pbExtraPoint12;             // Type 2 single extra point local coordinates
  ...
  GpuBuffer<double>* pbExtraPoint21;             // Type 1 dual extra point local coordinates
  ...
  GpuBuffer<double>* pbExtraPoint22;             // Type 2 dual extra point local coordinates
  GpuBuffer<int4>*   pbExtraPointCustomFrame;    // Remapped custom EP frames
  GpuBuffer<int>*    pbExtraPointCustomIndex;    // Remapped custom extra points
  GpuBuffer<double>* pbExtraPointCustom;         // Custom extra point frame measurements (this
                                                 //   is equivalent to the local coordinates of
                                                 //   other classes of EPs)
  ...
  GpuBuffer<int>*    pbImageEPCustomIndex;       // Remapped custom extra points, neighbor list
  GpuBuffer<int4>*   pbImageEPCustomFrame;       // Remapped custom EP frames, neighbor list
  ...
  GpuBuffer<int>*     pbSlowTIShakeID1;     // Central atom of XH4 Shake constraint
  GpuBuffer<int4>*    pbSlowTIShakeID2;     // XH8 SHAKE constraint hydrogens
  GpuBuffer<int4>*    pbSlowTIShakeID3;     // XH8 SHAKE constraint hydrogens
  GpuBuffer<double2>* pbSlowTIShakeParm;    // XH8 SHAKE central atom mass and equilibrium
                                            //   bond length
  GpuBuffer<double>*  pbSlowTIShakeInvMassH;// XH8 SHAKE HMR inverse hydrogen mass
  GpuBuffer<double2>* pbShakeParm2;         // SHAKE central atom mass and eq bond length for ti 2
  GpuBuffer<double2>* pbSlowShakeParm2;     // XH4 SHAKE central atom mass and eq bond length for ti 2
  GpuBuffer<double2>* pbSlowTIShakeParm2;   // XH8 SHAKE central atom mass and eq bond length for ti 2
  ...
  GpuBuffer<PMEMask>* pbExclusion;        // Exclusion masks
  ...
  bool bNCCL;               // Flag to indicate NCCL is used
  bool bPersistentP2P;      // Flag to indicate persistent allreduce is used
...
#ifdef NCCL
  ncclComm_t ncclComm;      // NCCL Communicator
#endif
#endif
#ifdef AWSMM
  // AWSMM stuff
