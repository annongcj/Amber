...
base_gpuContext::base_gpuContext() :
  ...
  bCPURandoms(false), gbsa(0), step(0), forwardPlan(0), backwardPlan(0),
#ifdef VKFFT
  appR2C(NULL),
#endif
  bPencilYShift(false), nonbond_skin(2.0), sortTempBytes(0),

  // middle scheme parameters
  ...
  pbPeerAccumulator(NULL), pbPeerAccumulatorList(NULL), pPeerAccumulatorMemHandle(NULL),
  pbPersistentStages(NULL),
  bCalculateLocalForces(false), bCalculateDirectSum(false), bCalculateReciprocalSum(false),
  bSingleNode(false), bP2P(false), bPersistentP2P(false),
#endif
  ...
  pbNMRr6avDistanceWeights(NULL), pbNMRr6avDistanceXYZ(NULL),
  ...
  pbSlowShakeInvMassH(NULL), pbUpdateIndex(NULL), pbSlowTIShakeID1(NULL),
  pbSlowTIShakeID2(NULL), pbSlowTIShakeID3(NULL), pbSlowTIShakeParm(NULL),
  pbSlowTIShakeInvMassH(NULL), pbShakeParm2(NULL), pbSlowShakeParm2(NULL),
  pbSlowTIShakeParm2(NULL),pbSGLDAverageBuffer(NULL),
  ...
  pbImageSlowTIShakeID1(NULL), pbImageSlowTIShakeID2(NULL), pbImageSlowTIShakeID3(NULL),
  ...
  pbNLNonbondCellStartEnd(NULL), pbFinishedBlocksCounters(NULL), pbKernelCompletionFlags(NULL),
  pbNLbSkinTestFail(NULL), pbNLCellHash(NULL), pbNLRecord(NULL), pbNLEntry(NULL),
  maxSoluteMolecules(0), maxPSSoluteMolecules(0), pbSoluteAtomID(NULL), pbSoluteAtomMass(NULL),
  pbSolute(NULL), pbUllSolute(NULL), pbSolventAtomID(NULL), pbSolvent(NULL), pbNTPData(NULL),
  pbConstraintAtomX(NULL), pbConstraintAtomY(NULL), pbConstraintAtomZ(NULL),
  pbConstraintCOMX(NULL), pbConstraintCOMY(NULL), pbConstraintCOMZ(NULL),
  ee_plasma(0.0), self_energy(0.0), vdw_recip(0.0), pbLJTerm(NULL),
#ifdef AWSMM
  ...
  //MCWat buffers
  pbStericRadius(NULL), pbStericGrid(NULL), pbStericThreadCount(NULL),
  pbCoarseGrid(NULL), pbRandomWater(NULL), pbCoarseTrans(NULL),
  pbCoarseGridEne(NULL), pbStericEmptyVoxels(NULL), pbVoxelOffset(NULL),

  // AMD buffers
  pbAMDfwgtd(NULL), pAmdWeightsAndEnergy(NULL),

  // SGLD buffers
  pbWsg(NULL),pbX0sg(NULL),pbX1sg(NULL),pbX2sg(NULL),pbRsg(NULL),pbFPsg(NULL),pbPPsg(NULL),
  pbImageWsg(NULL),pbImageX0sg(NULL),pbImageX1sg(NULL),pbImageX2sg(NULL),pbImageRsg(NULL),pbImageFPsg(NULL),pbImagePPsg(NULL),

  // GaMD buffers
  pbGaMDfwgtd(NULL), pGaMDWeightsAndEnergy(NULL),

  //Continuous pHMD data
  para(NULL), parb(NULL), park(NULL), barr(NULL), parmod(NULL), sp_par(NULL),
  qstate1(NULL), qstate2(NULL), vstate1(NULL), vstate2(NULL),
  qstate1_md(NULL), qstate2_md(NULL), charge_phmd(NULL), sp_grp(NULL),
  grplist(NULL), ph_theta(NULL), dph_theta(NULL), vph_theta(NULL),
  dphold(NULL), vphold(NULL), resbounds(NULL), pbRandomPHMD(NULL),
  baseres(NULL), excludeatoms(NULL), titratoms(NULL), ImageGrplist(NULL), ImageQstate1(NULL),
  ImageQstate2(NULL), ImageCharge_phmd(NULL), ImageVstate1(NULL),
  ImageVstate2(NULL), ImageIndexPHMD(NULL), linkgrplist(NULL), dudls(NULL)
{
  sim.InitSimulationConst();
}
...
base_gpuContext::~base_gpuContext()
{
  ...
  delete pbPeerAccumulatorList;
  delete[] pPeerAccumulatorMemHandle;
  delete pbPersistentStages;
#endif
  delete pbVel;
  delete pbLVel;
  delete pbCenter;
  ...
  delete pbImageSlowTIShakeID1;
  delete pbImageSlowTIShakeID2;
  delete pbImageSlowTIShakeID3;
  ...
  delete pbNLEntries;
  delete pbFinishedBlocksCounters;
  delete pbKernelCompletionFlags;
  ...
  // Delete MC Wat
  delete pbStericRadius;
  delete pbVoxelOffset;
  delete pbStericEmptyVoxels;
  delete pbStericGrid;
  delete pbStericThreadCount;
  delete pbCoarseGrid;
  delete pbRandomWater;
  delete pbCoarseTrans;
  delete pbCoarseGridEne;
  ...
  delete pbSlowTIShakeID1;
  delete pbSlowTIShakeID2;
  delete pbSlowTIShakeID3;
  delete pbSlowTIShakeParm;
  delete pbSlowTIShakeInvMassH;
  delete pbShakeParm2;
  delete pbSlowShakeParm2;
  delete pbSlowTIShakeParm2;
  ...
  cudaDestroyTextureObject(sim.texImageX2);
