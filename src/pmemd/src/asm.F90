...
!   Public subroutines:
!
!   asm_define                Sets up the calculation
!   asm_calc                  Launchs the string method calculation
!
!
...
module asm_mod
    ...
    use asm_CV_utilities_mod
    use asm_utilities_mod
    use asm_splines_utilities_mod

    use file_io_mod, only : nmlsrc
    use file_io_dat_mod, only : mdin, mdout
    use gbl_constants_mod, only : KB, error_hdr
    use pmemd_lib_mod, only : mexit
    use random_mod, only : amrand
    use mdin_ctrl_dat_mod, only : asm, dt, temp0
...
#include "mpif.h"
    ...
    double precision  :: force_scale !For gradual increase of forces during preparation
    double precision  :: gamma, position_gamma, force_gamma, force_kappa !friction
    double precision  :: Mav_damp !damping coefficient for Mav averaging
    double precision  :: RT !R*temperature
    double precision  :: K_d !Force constant for the d coordinate
    double precision  :: string_length !Arc-length of the string (using cubic splines)
    double precision  :: dpos, dK !displacement of the node position and the force constant
    double precision  :: mean_dx, mean_sigma2 !DEBUG
    ...
    !is essential for cubic splines interpolation to work.
    double precision, dimension(:), allocatable :: K_l, pos !Force constants and positions
    double precision, dimension(:), allocatable :: weight_buffer !Buffer with weights for reweighting to path CV
    double precision, dimension(:,:), allocatable :: string !Current string
    double precision, dimension(:,:), allocatable :: CV_buffer !Buffer for sampled points in CV space
    double precision, dimension(:,:), allocatable :: n !n vectors (tangents to the string)
    double precision, dimension(:,:), allocatable :: decomp_coef !coefficients for PMF decomposition
    double precision, dimension(:,:), allocatable :: CV !Current values of CVs at all nodes
    double precision, dimension(:), allocatable :: dz !Displacement vector for the string node
    ...
    double precision, dimension(:,:), allocatable :: Mav, Minv, B !Average M, inv(Mav), biasing matrices
    
    double precision, dimension(:,:,:), allocatable :: string_spline !Cubic splines interpolation of the string
    double precision, dimension(:,:,:), allocatable :: decomp_coef_spline !Cubic splines interpolation for PMF decomposition
    double precision, dimension(:,:,:), allocatable :: string_spline_smooth !Same, but for smooth normal vectors
    ...
    double precision  :: lambda !Inverse distance between two adjacent nodes
    logical :: z_bias !Whether the bias on the orthogonal direction should be removed for path CV
    logical :: reweight_z
    ...
    double precision, dimension(:), allocatable :: arc !arc length upto each node (for path CV)
    double precision, dimension(:), allocatable :: force, PMF !mean force and PMF along the path (using path CV)
    double precision, dimension(:,:), allocatable :: path !set of equidistant points along the string (with extrapolation)
    double precision, dimension(:,:), allocatable :: Minv_point !Metric tensor at each point
    !-------------------------
...
!==============================================================================
    ...
    !==================================================================
    subroutine asm_define(x_)
        
        double precision, dimension(3,natom), intent(in) :: x_
        double precision, dimension(natom*3) :: x

        integer :: i, j, unit, ifind
        double precision  :: force_constant, force_constant_d
        ...
        !Read asm namelist from input
        rewind(mdin)
        call nmlsrc('asm', mdin, ifind)

        if (ifind .ne. 0) then        ! Namelist found. Read it:
            read(mdin, nml = asm)
        else                          ! No namelist was present,
            write(mdout, '(a,a)') error_hdr, 'Could not find asm namelist'
            call mexit(6, 1)
          end if
        ...
        allocate(weight_buffer(buffer_size), CV_buffer(nCV,buffer_size))
        ...
        if (only_PMF .or. minimize) then
            ...
            if (.not. z_bias) force_constant_d = 0
            generate_pos = .false.
        end if
        ...
        write(mdout,*)
        write(mdout,"(A)")         "---String method------------------"
        write(mdout,"(A20,I15)")   "Number of CVs    ", nCV
        write(mdout,"(A20,I15)")   "Number of nodes  ", nnodes
        write(mdout,"(A20,I15)")   "Node             ", node
        write(mdout,"(A)")         "----------------------------------"
        write(mdout,*)
        write(mdout,*)
    ...
    contains
        ...
        subroutine read_initial_guess
            ...
            integer :: i, u, ninit
            double precision, dimension(:,:), allocatable :: string_init
            double precision, dimension(msize) :: Mtmp, Mtmpinv
        ...
        subroutine interpolate_linear(A, B, M)
            ...
            double precision  :: Ltotal, Lnew
            double precision, dimension(size(A, dim=2)) :: L
            double precision, dimension(size(A, dim=2),size(A, dim=2)) :: A_cont !continuous string (no jumps because of periodic coordinates)
    ...
    !==================================================================
    subroutine asm_calc(x_, force, energy, full_force)
    
        double precision, dimension(3,natom), intent(in) :: x_
        double precision, dimension(:,:), intent(out) :: force
        double precision, intent(out) :: energy
        double precision, dimension(3,natom), intent(in), optional :: full_force

        double precision, dimension(natom*3) :: x
        double precision :: dpos_tmp, dK_tmp, pos_target, delta, sigma2, sigma2_target
        double precision :: s, z
        double precision, dimension(nCV) :: dz_tmp
    ...
    contains
        ...
        double precision function scale_dpos(x) !Ensures that the node positions never cross
        ...
        subroutine average_string(first, last)
            ...
            integer :: i, u, n
            double precision, dimension(nnodes) :: params_tmp
            double precision, dimension(nCV) :: reference
            double precision, dimension(nCV, nnodes) :: string_tmp
            character*200 :: si
        ...
        subroutine add_force_ld !Elliptic potential (for the string method itself)
            ...
            integer :: i, j
            double precision, dimension(nCV) :: gradld !Gradient of l+d in CV space
        ...
        subroutine add_force_sz !Potential along path CV (for precise PMF calculation)
            ...
            integer :: i, j
            double precision, dimension(3,n_used_atoms) :: grad_s, grad_z, grad
        ...
        subroutine calc_grote_hynes
            ...
            integer :: i, j
            double precision :: sqm !sqrt(m)
            double precision :: dqm2 !|dq|**2 in mass-weighted cartesians
            double precision :: mu !reduced mass
            double precision :: f_q !projection of external force on q
            double precision :: fi_q !projection of fi on q
            double precision :: s, z !Not actually used, but needed as dummy
            double precision, dimension(3) :: dq_dxi !dq / dx_i mass-weighted
            double precision, dimension(3) :: fi ! - dV / dx_i mass-weighted
            double precision, dimension(3,n_used_atoms) :: grad_s
    ...
    subroutine write_convergence
        ...
        double precision, dimension(nCV,nnodes) :: tmp_string
    ...
    contains
        ...
        !----------------------------------------
        double precision function strings_distance(A, B, M_list)
    
            double precision, dimension(:,:), intent(in) :: A, B
            double precision, dimension(:,:), intent(in) :: M_list
    ...
    subroutine update_B
        ...
        double precision, dimension(nCV) :: Minvn
        double precision, dimension(msize) :: S
    ...
    subroutine REX
        ...
        double precision :: rnd, dpos_tmp
        double precision, dimension(nCV) :: dz_tmp
        logical, dimension(nnodes) :: exchange
        double precision, dimension(buffer_size) :: buffer_tmp
        double precision, dimension(nCV,buffer_size) :: CV_buffer_tmp
    ...
    contains
        ...
        !---------------------------------------
        double precision function prob(i) !Calculates the exchange probability between nodes i and i+1
        ...
        !---------------------------------------
        double precision function node_energy(node_idx, CV_idx)
    ...
    subroutine reparameterize_linear
        ...
        integer :: i, j
        double precision, dimension(nnodes) :: L
        double precision, dimension(nCV) :: dz
    ...
    subroutine reparameterize_splines
        ...
        integer :: i
        double precision, dimension(nnodes) :: x !Initial parameterization (arbitrary, node indices are used here)
        double precision, dimension(nnodes) :: L !arc lengths up to each node
        double precision, dimension(nnodes) :: string_der !|dz/dx|_M - string derivative length with metric Minv
        double precision, dimension(nnodes-1,5) :: string_der_spline !|dz/dx|_M as function of x (to calculate the arc length)
        double precision, dimension(nnodes-1,5) :: xofL_spline !x as function of arc length
    ...
    !Create splines interpolation for PMFCV decomposition
        ...
        double precision, dimension(:), intent(in) :: L !arc lengths up to each node
...
!------------------------------------------------------------------------------!
    ...
    subroutine update_path
        ...
        integer :: i, j
        double precision, dimension(nCV) :: neg_extra, pos_extra !Vectors to extrapolate the path
    ...
    subroutine read_pathCV
        ...
        double precision, dimension(nCV, nCV) :: Mbuf
    ...
    subroutine string_PMF_calculate
        ...
        double precision :: mean, sigma2, bin_width, mean_error, sigma2_error
        double precision :: K, s0, tmp
        integer, dimension(nbins) :: N_bin !Number of points in each bin
        double precision, dimension(nbins) :: force, t_bin
        double precision, dimension(nbins) :: pop_bin !Bin populations
        double precision, dimension(nbins) :: w_error, force_error, PMF_error
        double precision, dimension(nCV) :: CVtmp
        double precision, dimension(:), allocatable :: s, z, w
    ...
    contains
        ...
        subroutine string_PMFCV_write(filename)
                    ...
                    double precision, dimension(nCV) :: coef !Coefficients for the decomposition (Minv*n)
                    double precision, dimension(nCV) :: tmp
                    double precision, dimension(nCV,nbins) :: forceCV, PMFCV, forceCV_error, PMFCV_error
    ...
    !Calculation of a metric-corrected path CV (J. Comput. Chem. 2014, 35, 1672–1681)
        ...
        double precision, dimension(nCV), intent(in) :: CV
        double precision, intent(out) :: s
        double precision, intent(out), optional :: z
        double precision, dimension(natom*3,nCV), intent(in), optional :: Jacobian
        double precision, dimension(3,n_used_atoms), intent(out), optional :: grad_s, grad_z

        integer :: i, j
        double precision :: d, sumw
        
        double precision, dimension(npointstotal) :: w
        double precision, dimension(nCV) :: dCV
        double precision, dimension(nCV,npointstotal) :: grad_tmp !temporary values for gradient calculation
