...
use nfe_lib_mod, only : SL => STRING_LENGTH, STSM_OUTPUT_UNIT, STSM_CV_UNIT
...
double precision, private, allocatable, save :: cv_copy(:)

double precision, private, allocatable, save :: f_cv(:)

double precision, private, allocatable, save :: x_eq(:)

double precision, private, allocatable, save :: all_cv(:)
double precision, private, allocatable, save :: all_anchor(:)
...
character(SL), private, save :: MDOUT_FILE
...
double precision, private, save :: smooth
...
#  endif /* MPI */
   ...
   run_freq = min(run_freq, pmemd_nstlim())
   ...
   if (id_image.eq.0) &
#  ifdef MPI
      id_image = mod(master_rank,num_copies)+1
#  else
      id_image = 1
#  endif /* MPI */
   id_image = id_image - 1
...
#  ifdef MPI
      if (multipmemd_numgroup().gt.1) then
        ...
        ! get all image ids
        allocate(all_image(multipmemd_numgroup()), stat = error)
        call mpi_allgather(id_image, 1, &
            MPI_INTEGER, all_image, 1, &
            MPI_INTEGER, pmemd_master_comm,error)
        ...
        call mpi_bcast(num_images, 1, MPI_INTEGER, 0, pmemd_master_comm, error)
        nfe_assert(error.eq.0)
        allocate(all_anchor(ncolvars*num_images), stat = error)
        allocate(all_cv(ncolvars*multipmemd_numgroup()), stat = error)
      end if
#  endif /* MPI */
...
#  endif /* MPI */
   ...
   call flush_UNIT(stsm_UNIT, output_file)
   ...
   write (unit = LOG_UNIT, &
      fmt = '(a,a,'//pfmt(multipmemd_numgroup()/num_images)//')') &
        NFE_INFO, 'number of copies per image = ', multipmemd_numgroup()/num_images
...
subroutine on_force(x, f, pot)
   ...
   double precision :: curlen
   double precision :: ratio
   double precision, allocatable :: totlen(:)
   ...
   if (mod(mdstep,run_freq).eq.0) then
      if (mod(mdstep/run_freq,num_repeats).eq.0) then
         write (unit = OUT_UNIT, fmt = '(/a,a)') &
            NFE_INFO,'#   new restarint:'
      else 
         write (unit = OUT_UNIT, fmt = '(/a,a)') &
            NFE_INFO,'#   restoring restarint:'
      end if
      do n = 1, ncolvars
...
!	       Drift
               ...
               if (report_drift) then
                  do n = 0, num_images-1
                     ...
                     call flush_UNIT(OUT_UNIT, MDOUT_FILE)
                  end do
               end if
!              Smoothing
...
!	    <<broadcast the new centers>>
            ...
            a_position(1:ncolvars) = &
              all_anchor(all_image(master_rank+1)*ncolvars+1&
              : (all_image(master_rank+1)+1)*ncolvars)
         else
            a_position(:) = cv_copy(:)
         end if
