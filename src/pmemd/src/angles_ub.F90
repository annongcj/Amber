...
! energy...
    ...
    if (ti_mode .ne. 0) then
      ti_region = 0
      if (ti_lst(1,i) .eq. 1 .or. ti_lst(1,k) .eq. 1) then
        ti_region = 1
      else if (ti_lst(2,i) .eq. 1 .or. ti_lst(2,k) .eq. 1) then
        ti_region = 2
      end if
      if (ti_region .gt. 0) then
        ...
        if (gbl_atm_owner_map(i) .eq. mytaskid) then
          ...
          if (ti_mode .eq. 1 .or. ti_lscale) then !linear scaling    
            call ti_update_ene(df * da, si_angle_ub_ene, ti_region, 2)
            !no need to scale da as we always scale df
          else
            ti_ene(ti_region,si_angle_ub_ene) = &
              ti_ene(ti_region,si_angle_ub_ene) + df * da
            da = 0.d0
          end if
#ifdef MPI
        end if
