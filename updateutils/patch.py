...
occasionally upgrades) to Amber
"""
...
import sys
from updateutils.exceptions import (
    PatchNotFound, NoPatchHeader, IllegalFile, BadPatchError, MissingProgram,
    PatchingError, UpdaterTypeError, PatchingWarning, PatchingUpdater)
...
class LocalPatch(object):
   ...
   """
    ...
    def __init__(self, patch_file, prefix='bugfix'):
        ...
        try:
            self.number = int(
                path.split(self.name)[1].replace(prefix + '.', ''))
        except (ValueError, TypeError):
            self.number = 0
        if not path.exists(patch_file):
            raise PatchNotFound(
                'Could not open local patch file %s' % self.name)
        ...
        for line in tmpfile:
            ...
            if end_line.match(line.strip()) and not end_line.sub(
                    '', line.strip()):
                tmpfile.close()
                header_ended = True
                break
    ...
    def _files_edited(self):
      ...
      """
        ...
        delfile = re.compile(r'deleted file mode (\d{3})+')
            ...
            if selffile.match(line):
                ...
                if filename.startswith('../'):
                    ...
                    if (oldfname in retdict['new files'] or
                        ...
                        raise BadPatchError(
                            'Deleted file appears multiple times.')
                    ...
                    if oldfname == '/dev/null':
                        if (filename in retdict['new files'] or
                            ...
                            raise BadPatchError(
                                'New file appears multiple times.')
                        ...
                        if (filename in retdict['new files'] or
                            ...
                            raise BadPatchError(
                                'New/Deleted file appears multiple '
                                'times.')
                        if not filename in retdict['modified files']:
                            retdict['modified files'].append(filename)
                oldfname = ''
    ...
    def restore_backups(self, remove_dir=True, reverse=False):
      ...
      """
        if not hasattr(self, 'backup_dest'):
            warnings.warn('No backup file directory to restore',
                          PatchingWarning)
            return
      ...
      """
        ...
        if patch is None:
            raise MissingProgram(
                'Cannot find patch. Install patch from your '
    ...
    def _fix_modes(self, reverse=False):
      ...
      """
        ...
        if len(self.files[permkey]) == 0:
            warnings.warn(
                ('No permission information given in %s! All new files '
                 'will be read-only!') % self.name, PatchingWarning)
            return
        if len(self.files[filekey]) != len(self.files[permkey]):
            warnings.warn(
                ('Permissions not found for every file in %s. All new '
    ...
    def safe_apply(self, reverse=False, ignore_prev_applied=True):
      ...
      """
        ...
        if not succeeded:
            raise PatchingError(
                ('%s failed to apply. No changes made from this '
                 'patch') % self.name)
...
class CompressedLocalPatch(LocalPatch):
   ...
   """
    ...
    def __init__(self, patch_file, prefix='bugfix'):
        ...
        LocalPatch.__init__(self,
                            patch_file.replace(self.extension, ''), prefix)
...
def test():
    ...
    print('Del files: ' +
          '\n           '.join(patch1._files_edited()['deleted files']))
    print('Others   : ' +
          '\n           '.join(patch1._files_edited()['modified files']))
    ...
    print('Del files: ' +
          '\n           '.join(patch2._files_edited()['deleted files']))
    print('Others   : ' +
          '\n           '.join(patch2._files_edited()['modified files']))
    ...
    print('Del files: ' +
          '\n           '.join(patch3._files_edited()['deleted files']))
    print('Others   : ' +
          '\n           '.join(patch3._files_edited()['modified files']))
    ...
    remote = urlopen('https://ambermd.org/bugfixes/AmberTools/12.0/bugfix.1')
