      ...
      """
        ...
        for j, i in enumerate(patches_to_print):
            ...
            if self._to_print['description']:
                prbuf += 'Description:\n' + self.patch_list.description(
                    i, fmt=True)
                prbuf += '\n'
            if self._to_print['files']:
                prbuf += 'Files edited: %s\n' % (
                    '\n              '.join(self.patch_list.files_edited(i)))
...
class RemotePatchRepository(PatchRepository):
    ...
    def __init__(self, location, title, prefix='bugfix'):
        ...
        if not (self.location.startswith('http://') or
                self.location.startswith('ftp://') or
                self.location.startswith('https://')):
            raise RepoLocationError(
                'RemotePatchRepository cannot point to a local'
                ' directory')
    ...
    def set_destination_repo(self, other):
      ...
      """
        if not isinstance(other, LocalUnappliedPatchRepository):
            raise APIError(
                'Can only download to a LocalUnappliedPatchRepository')
        ...
        for i, num in enumerate(self.patch_list.numbers):
            ...
            if num >= first and num <= last:
                ...
                if ndl == 0:
                    first_dl = num
                    self.output.write(
                        'Downloading updates for %s\n' % self.reponame)
                ndl += 1
                # Replace .bz2_ with .bz2 to account for patch_amber.py fix.
                pname = os.path.split(self.patch_list[i])[1].replace(
                    '.bz2_', '.bz2')
   ...
   """
    ...
    def __init__(self, location, title, prefix='bugfix'):
        ...
        try:
            ...
            if self.absolute:
                raise FileNotFound(
                    'Patch directory with absolute path does not '
      ...
      """
        ...
        for i in iterseq:
            ...
            if self.patch_list.disordered:
                ...
                if patches_self:
                    raise PatchingUpdater(
                        'DETECTED A PATCH TO THE UPDATER SCRIPT. '
   ...
   """
    ...
    def empty_repo(self):
      ...
      """
        ...
        if self.absolute:
            raise APIError(
                'Cannot delete files from a repository with an absolute'
   ...
   """
    ...
    def set_destination_repo(self, other):
      ...
      """
        if not isinstance(other, LocalUnappliedPatchRepository):
            raise APIError(
                'Can only unapply to a LocalUnappliedPatchRepository')
        if other.prefix != self.prefix:
            raise APIError(
                'Can only unapply to a LocalUnappliedPatchRepository '
                'with the same file name prefix')
...
def test():
    ...
    AT_remote = RemotePatchRepository(
        ...
        'AmberTools 12',
        prefix='bugfix')
    AT_local_unap = LocalUnappliedPatchRepository(
        os.path.join('.patches', 'AmberTools_Unapplied_Patches'),
        'AmberTools 12',
        prefix='bugfix')

    AT_local_ap = LocalAppliedPatchRepository(
        os.path.join('.patches', 'AmberTools_Applied_Patches'),
        'AmberTools 12',
        prefix='bugfix')
