...
#Example use:
#
# ./test_amber_hip_parallel.x         (This will run with defaults)
# ./test_amber_hip_parallel.x DPFP (This selects the hybrid DPFP
#                                    precision model to use)
#
# ./test_amber_hip_parallel.x SPFP
...
logdir="${AMBERHOME}/logs/test_amber_hip_parallel"
logprefix="${logdir}/${date_string}"
logfile="${logprefix}.log"
difffile="${logprefix}.diff"

mkdir -p ${logdir}
(make -k -f Makefile test.parallel.hip PREC_MODEL=$PREC_MODEL 2>&1) | tee ${logfile}
...
error_count=`grep --text "[Ee]rror" ${logfile} | wc -l`
