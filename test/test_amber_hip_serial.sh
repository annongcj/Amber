...
#This script is for testing the hip implmentation via the test
...
#Example use:
#
# ./test_amber_hip.x         (This will run with defaults)
# ./test_amber_hip.x DPFP (This selects all double precision as the
#                              precision model)
#
# ./test_amber_hip.x SPFP  (This will run the hybrid SPFP precision model)

export AMBERHOME=$PMEMDHOME
. `dirname $0`/../test/test_check.sh
...
mkdir -p ${logdir}
(make -k -f Makefile test.hip.serial2 PREC_MODEL=$PREC_MODEL 2>&1) | tee ${logfile}
(make -k -f Makefile finished.hip.serial 2>&1) | tee -a ${logfile}

passed_count=`grep --text PASS ${logfile} | wc -l`
questionable_count=`grep --text "FAILURE:" ${logfile} | wc -l`
questionable_ignored_count=`grep --text -e "FAILURE: (ignored)" ${logfile} | wc -l`
error_count=`grep --text "Program [Ee]rror" ${logfile} | wc -l`
